...
export default function ExperimentalError({ error, reset }: { error: Error & { digest?: string }; reset: () => void }) {
  useEffect(() => {
    console.error("Experimental /x/ area error:", error);
  }, [error]);

  return (
    <div className="px-4 py-12 sm:px-6">
      <div className="mx-auto max-w-md rounded-2xl border theme-border bg-[var(--surface)] p-6 text-center">
        <div className="mx-auto mb-3 inline-block rounded-full bg-[var(--surface-2)] px-3 py-1 text-xs font-medium theme-text-muted">/x/ experimental</div>
