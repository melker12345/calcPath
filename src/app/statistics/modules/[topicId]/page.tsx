...
const border = "rgba(232,228,217,0.12)";
  ...
  if (!module || !topic) {
    return (
      <div className="mx-auto w-full max-w-3xl px-4 py-8 sm:px-6 sm:py-12">
        ...
        <Link className="btn-stats-secondary mt-4 inline-flex" href="/statistics/modules">
          Back to modules
        </Link>
      </div>
    );
    ...
    <div className="mx-auto flex w-full max-w-4xl justify-center px-4 py-8 sm:px-6 sm:py-12">
      <div className="w-full">
        <div className="mb-6 space-y-3 sm:mb-8">
          <Link className="text-sm font-medium hover:underline" href="/statistics/modules" data-no-print style={{ color: accent }}>
            ← Back to modules
          </Link>
                    ...
                    {printStep === "choose" ? (
                      <>
                        ...
                        <button type="button" onClick={() => { setShowPrintModal(false); setPrintStep("choose"); setPrintMode("text"); }} className="flex w-full items-center gap-2.5 rounded-lg px-3 py-2.5 text-left text-sm font-medium text-zinc-700 transition hover:bg-zinc-50">
                          <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-lg bg-amber-100 text-xs">📄</span>
                          Text only
                        </button>
                      ...
                      <>
                        <button type="button" onClick={() => setPrintStep("choose")} className="mb-2 text-xs font-medium text-amber-600 hover:text-amber-800">← Back</button>
        ...
        {/* Main content */}
        <article className="rounded-2xl p-5 shadow-xl sm:rounded-3xl sm:p-10" style={{ background: "rgba(255,255,255,0.03)", border: `2px solid ${border}` }}>
              ...
              <div key={section.title}>
                ...
                <div>
                  ...
                  {section.eli5 && section.eli5.length > 0 && (
                    <details className="print-keep-together group mt-6 rounded-2xl" style={{ border: `2px solid rgba(253,230,138,0.2)`, background: "rgba(253,230,138,0.05)" }}>
                      <summary className="flex cursor-pointer select-none items-center gap-3 px-5 py-3.5 text-sm font-semibold [&::-webkit-details-marker]:hidden" style={{ color: accent }}>
                        <span className="flex h-7 w-7 items-center justify-center rounded-lg text-base transition-transform group-open:rotate-12" style={{ background: "rgba(253,230,138,0.15)" }}>💡</span>
        ...
        <nav className="mt-6 grid gap-4 sm:mt-8 sm:grid-cols-2" data-no-print>
          {prevTopic ? (
            <Link href={`/statistics/modules/${prevTopic.id}`} className="group rounded-2xl p-5 transition-all hover:shadow-md sm:rounded-3xl sm:p-6" style={{ background: "rgba(255,255,255,0.03)", border: `2px solid ${border}` }}>
          ...
          {nextTopic ? (
            <Link href={`/statistics/modules/${nextTopic.id}`} className="group rounded-2xl p-5 text-right transition-all hover:shadow-md sm:rounded-3xl sm:p-6" style={{ background: "rgba(255,255,255,0.03)", border: `2px solid ${border}` }}>
