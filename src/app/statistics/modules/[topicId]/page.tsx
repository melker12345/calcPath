  ...
  return (
    <div className="mx-auto w-full max-w-[760px] px-4 py-8 sm:px-6 sm:py-10">
      <div className="min-w-0">
        ...
        <div className="mt-10">
          <ModuleDoneButton moduleId={`statistics:${topic.id}`} />
        </div>

        <nav className="mt-10 grid gap-4 border-t border-stone-300 pt-8 sm:grid-cols-2" data-no-print>
          {prevTopic ? (
            <Link href={`/statistics/modules/${prevTopic.id}`} className="border border-stone-300 bg-[#fffef8] p-5 transition hover:bg-stone-100 nav-card">
          ...
          {nextTopic ? (
            <Link href={`/statistics/modules/${nextTopic.id}`} className="border border-stone-300 bg-[#fffef8] p-5 text-right transition hover:bg-stone-100 nav-card">
