...
export default function TestClient({ subjectSlug = "calculus", topicId, topic, allTestQuestions }: TestClientProps) {
  // AuthProvider removed (global auth strip); ProgressProvider kept so the rich test UI (which uses useProgress) never pulls the
  ...
  return (
    <ProgressProvider>
      <TestClientInner subjectSlug={subjectSlug} topicId={topicId} topic={topic} allTestQuestions={allTestQuestions} />
    </ProgressProvider>
  );
}

function TestClientInner({ subjectSlug, topicId, topic, allTestQuestions }: TestClientProps) {
  ...
  if (!topic) {
    return (
      <div className="mx-auto w-full max-w-3xl px-4 py-8 sm:px-6 sm:py-12">
        ...
        <Link className="btn-secondary mt-4 inline-flex" href={`/${subjectSlug}`}>
          Back to {subjectSlug === "calculus" ? "Calculus" : "Subject"}
        </Link>
      </div>
    );
  }
  ...
  if (allTestQuestions.length === 0) {
    return (
      <div className="mx-auto w-full max-w-3xl px-4 py-8 sm:px-6 sm:py-12">
        ...
        <Link className="btn-secondary mt-4 inline-flex" href={`/${subjectSlug}/modules/${topic.id}`}>
          Back to module
        </Link>
      </div>
    );
      ...
      <div className="mx-auto w-full max-w-4xl px-4 py-8 sm:px-6 sm:py-12">
        ...
        <div className="mt-8 space-y-4">
          {(["easy", "medium", "hard"] as const).map((difficulty) => {
            const diffResults: ResultWithMeta[] = results
              .map((r, i): ResultWithMeta => ({ ...r, index: i, question: testQuestions.find(q => q.id === r.questionId) }))
