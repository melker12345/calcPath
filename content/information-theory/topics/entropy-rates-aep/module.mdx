...
For a single random variable, entropy is the uncertainty per draw. A *process* $X_1, X_2, X_3,\dots$ is different: its symbols are usually dependent, so the useful question is not how uncertain one symbol is but how much *new* uncertainty each additional symbol contributes. That long-run average per symbol is the entropy **rate**, and it, not the single-symbol entropy, is the number that governs the compressibility of real files — whose symbols are anything but independent.
...
:::figure[For a stationary process the one-step conditional entropies $H(X_n\mid X_1,\dots,X_{n-1})$ fall monotonically to the entropy rate, while the block averages $\tfrac1n H(X_1,\dots,X_n)$ — their Cesàro means — approach the same limit from above and more slowly. Here the limit is $0.8$ bits per symbol.]{#entropy-rate-figure}
{
  ...
  "marks": [
    ...
    { "kind": "text", "at": [2.0, 0.83], "text": "entropy rate H(X) = 0.8", "leaderTo": [3.0, 0.8] }
  ]
}
:::
...
From either state the next symbol is a Bernoulli$(0.1)$ decision to flip, so $H(X_2\mid X_1 = i) = h_2(0.1) \approx 0.469$ for each $i$, and
...
From state $0$ the next symbol is Bernoulli$(0.2)$, contributing $h_2(0.2) \approx 0.7219$; from state $1$ it is Bernoulli$(0.4)$, contributing $h_2(0.4)\approx 0.9710$. Weighting,

$$H(\mathcal X) = \tfrac23(0.7219)+\tfrac13(0.9710) \approx 0.4813+0.3237 = 0.805 \text{ bits/symbol}.$$

Sanity check: the marginal entropy is $h_2(1/3)\approx 0.918$ bits, and indeed $0.805 < 0.918$. □
:::
...
Write $A_\varepsilon^{(n)} \subseteq \mathcal X^n$ for the set of sequences whose per-symbol code length $-\tfrac1n\log_2 p(x_1,\dots,x_n)$ lies within $\varepsilon$ of $H$ — the **typical set**. It is defined carefully, and its size pinned down, in Definition *Typical set* below; here we need only the name.
...
and consequently $P\bigl(A_\varepsilon^{(n)}\bigr) \to 1$ for every $\varepsilon>0$: the typical set eventually carries essentially all of the probability.
:::
...
an average of i.i.d. random variables with common mean $\mathbb{E}[-\log_2 p(X)] = H(X)$ and finite variance (the alphabet is finite). The weak law of large numbers gives convergence in probability to $H$, which is the first claim, and membership of $A_\varepsilon^{(n)}$ is precisely the event $\bigl|-\tfrac1n\log_2 p(X^n) - H\bigr| \le \varepsilon$, so its probability tends to $1$. The weak law itself is proved in the next section, and the counting consequences — that $|A_\varepsilon^{(n)}|$ is about $2^{nH}$ — in the section after that.
:::
...
:::solution
The block entropy is about $nH(\mathcal X) = 1.2\times10^{6}$ bits $=150{,}000$ bytes $=150$ kB, and no scheme does sustainably better — that is the converse half of the source coding theorem, proved in the next chapter.
...
where $S$ is the support of $f$, whenever the integral exists. (The lower-case $h$ here is the traditional notation and unfortunately collides with the binary entropy function $h_2(p)$; the argument tells them apart — a random variable means differential entropy, a number in $[0,1]$ means the binary entropy.)
:::
...
:::summary
So far: for a stationary process the entropy rate exists, equals the limiting one-step conditional entropy, is at most the marginal entropy with equality only in the i.i.d. case, and equals $-\sum_{i,j}\mu_iP_{ij}\log_2 P_{ij}$ for a stationary Markov chain. The AEP says that a long realisation exhibits that rate, and differential entropy is the continuous analogue that keeps the formula but loses the interpretation. The rest of the chapter makes the AEP quantitative: where it comes from, how large the typical set is, and what compressor it hands us.
:::
...
Recall the entropy rate $H(\mathcal X) = \lim_{n\to\infty} \tfrac1n H(X_1,\dots,X_n)$ of Definition *Entropy rate*, which for a stationary process also equals $\lim_{n\to\infty} H(X_n\mid X_1,\dots,X_{n-1})$, the conditional entropy of the next symbol given the entire past. That is an average over the whole ensemble; what a compressor actually meets is one string, so we need a per-string quantity beside it.

For an i.i.d. source the chain rule gives $H(X_1,\dots,X_n) = nH(X)$ immediately, so $H(\mathcal X) = H(X)$: the entropy rate is the ordinary single-symbol entropy. For a source with memory the two differ, and the difference is exactly the redundancy that a model of the memory can harvest.
...
The entropy rate is an expectation, $H(\mathcal X) = \lim_{n\to\infty} \frac1n E[-\log p(X^n)]$; the sample information rate is the random quantity being averaged. The AEP says that the random quantity concentrates on its mean — that essentially every long string costs the same number of bits per symbol.
...
:::theorem[AEP, quantitative form]{#aep-quantitative}
Let $X_1,X_2,\dots$ be i.i.d. $\sim p$ on a finite alphabet, write $H = H(X)$, and let

$$V = \operatorname{Var}\bigl(-\log p(X)\bigr) = \sum_x p(x)\log^2 p(x) \;-\; H^2$$

be the **varentropy** of the source. Then for every $\varepsilon>0$ and every $n$,

$$\Pr\!\left( \Bigl| -\frac1n\log p(X^n) - H \Bigr| \ge \varepsilon \right) \;\le\; \frac{V}{n\varepsilon^{2}} .$$

Letting $n\to\infty$ recovers Theorem *Asymptotic equipartition property, i.i.d. case*, now with an explicit rate.
:::
...
and their common variance is $V$, by the definition above. So $-\frac1n\log p(X^n)$ is exactly the sample mean $\bar Y_n$ of Theorem *Weak law of large numbers* with $\mu = H$ and $\sigma^2 = V$, and the Chebyshev step of that proof gives the displayed bound — at every $n$, not merely in the limit.
:::

That is the whole trick, and it is worth saying twice: **the AEP is the weak law of large numbers applied to the random variable "surprise".** The varentropy is the constant that says how long a block must be before the law bites. A nearly uniform source has small $V$ and concentrates almost at once; a source with one rare, very surprising symbol has large $V$ and needs long blocks before its typical set is worth anything. That is why the asymptotic statements of this chapter are careful to be asymptotic.
...
$$-\frac1n\log p(X^n) \longrightarrow H(\mathcal X) \quad \text{almost surely.}$$
...
:::intuition
Toss a biased coin a million times. You cannot predict the sequence, but you *can* predict its cost: almost every outcome you will ever see takes about the same number of bits to write down, namely $H(\mathcal X)$ per toss.
...
The AEP is a statement about a limit in probability. To use it we convert it into a statement about a *set* — the strings whose rate is close to $H(\mathcal X)$.
...
For any source satisfying the AEP with entropy rate $H(\mathcal X)$, and any $\varepsilon>0$, there exist lossless codes with expected description length at most $H(\mathcal X)+\varepsilon$ bits per symbol for all sufficiently large block lengths.
:::
