...
a vacuous bound — which is the right answer, since $0.8$ bits of residual uncertainty is consistent with a very good decoder. The sharp form $0.8 \le h_2(P_e)+P_e\log_2 7$ gives $P_e \gtrsim 0.15$, a real constraint. □
:::
...
:::solution
For a fair input into a binary symmetric channel with crossover $p$, the output is also a fair bit, so $I(X;Y)=H(Y)-H(Y|X) = 1 - h_2(p)$. With $p=0.1$, $h_2(0.1)\approx 0.469$, so $I(X;Y)\approx 0.531$ bits.

The two hops compose into one channel that flips the bit exactly when precisely one of the two hops flips it, so the effective crossover is $p' = 0.1(0.9)+0.9(0.1)=0.18$. Hence $I(X;Z) = 1-h_2(0.18) \approx 1-0.680 = 0.320$ bits.
