...
:::figure[The binary entropy $h_2(p)$. It vanishes at the deterministic ends $p=0$ and $p=1$, peaks at $h_2(\tfrac12)=1$ bit, and is concave throughout — so a mixture of two coins is never more predictable than the mixture of their entropies.]{#binary-entropy-figure}
{
  "type": "plot",
  "x": [0.0, 1.0],
  "y": [0.0, 1.25],
  ...
  "marks": [
    ...
    { "kind": "text", "at": [0.17, 0.16], "text": "h_2(0.1) = 0.469", "leaderTo": [0.1, 0.469] },
    { "kind": "text", "at": [0.72, 0.62], "text": "h_2(0.25) = 0.811", "leaderTo": [0.25, 0.8113] }
  ]
}
:::
...
the entropy of a single bit that equals $1$ with probability $p$. It satisfies $h_2(p) = h_2(1-p)$, $h_2(0) = h_2(1) = 0$, and $h_2(1/2) = 1$.
:::
...
$$H(X) = \frac{h_2(p)}{p} \quad\text{bits},$$
...
$$H(X) = -\frac{q}{p}\log_2 q - \log_2 p = \frac{-q\log_2 q - p\log_2 p}{p} = \frac{h_2(p)}{p}.$$
:::
...
:::solution
Here $p = 1/2$, so $H = h_2(1/2)/(1/2) = 1/0.5 = 2$ bits. The unary code — $k-1$ zeros followed by a one — has expected length $\mathbb{E}[X] = 2$ bits and is therefore optimal, which we can confirm against the source coding theorem: $H = 2 \le L^* = 2 < 3$. Equality is possible because every probability $2^{-k}$ is dyadic. □
:::
...
:::solution
Value by value: $H(Y|X{=}0) = 0$ because $Y$ is deterministic there, and $H(Y|X{=}1) = h_2(1/2) = 1$. Averaging,
...
Going the other way: $P(Y{=}0)=\tfrac34$ and $P(Y{=}1)=\tfrac14$, so $H(Y) = h_2(1/4) \approx 0.811$ bits and $H(X|Y) = H(X,Y)-H(Y) \approx 1.5-0.811 = 0.689$ bits.
...
:::pitfall
"Conditioning reduces entropy" is a statement about the **average** $H(Y|X)$, not about individual values. It is entirely possible that $H(Y\mid X=x) > H(Y)$ for some particular $x$: a specific observation can leave you *more* confused than you were. Concretely, let $P(X{=}a)=0.8$ with $P(Y{=}1\mid X{=}a)=1$, and $P(X{=}b)=0.2$ with $P(Y{=}1\mid X{=}b)=\tfrac12$. Then $P(Y{=}1)=0.8(1)+0.2(0.5)=0.9$, so $H(Y)=h_2(0.9)\approx 0.469$, while $H(Y\mid X{=}b)=1 > H(Y)$. The average is safe: $H(Y\mid X)=0.8(0)+0.2(1)=0.2 \le 0.469$.
:::
...
$$H(Y \mid X) \ \le\ h_2(P_e) + P_e \log_2\bigl(|\mathcal{Y}| - 1\bigr) \ \le\ 1 + P_e\log_2|\mathcal{Y}|.$$
...
$$H(E,Y\mid\hat Y) = \underbrace{H(E\mid \hat Y)}_{\le\, h_2(P_e)} + H(Y\mid E,\hat Y) = \underbrace{H(Y\mid\hat Y)}_{\text{what we want}} + \underbrace{H(E\mid Y,\hat Y)}_{=\,0},$$

the last term vanishing because $E$ is a function of $(Y,\hat Y)$, and the first being at most $H(E) = h_2(P_e)$ since conditioning reduces entropy.
...
$$H(Y\mid\hat Y) \le h_2(P_e) + P_e\log_2(|\mathcal Y|-1).$$

Finally, since $\hat Y = g(X)$ is a function of $X$, conditioning on $X$ is at least as informative as conditioning on $\hat Y$: $H(Y\mid X) = H(Y\mid X,\hat Y) \le H(Y\mid \hat Y)$, the equality because $\hat Y$ adds nothing to $X$ and the inequality because conditioning reduces entropy. The weaker second form follows from $h_2(P_e)\le 1$ and $\log_2(|\mathcal Y|-1) \le \log_2|\mathcal Y|$, and rearranging gives the bound on $P_e$.
:::
...
So *every* decoder errs at least $37.5\%$ of the time. The sharper form $4 \le h_2(P_e) + P_e\log_2 255$ gives a slightly better bound: at $P_e = 0.42$ the right side is $0.9815 + 0.42(7.994) \approx 4.34 \ge 4$, while at $P_e = 0.375$ it is $0.9544+2.998 \approx 3.95 < 4$, so in fact $P_e \gtrsim 0.38$. Sanity check: $H(Y)=8$ bits and $H(Y|X)=4$ means the observation conveyed $I(X;Y)=4$ bits — half the message — and half a message is not enough to decode it. □
:::
...
with equality in the last display exactly under mutual independence, and Fano's inequality $H(Y|X) \le h_2(P_e)+P_e\log_2(|\mathcal Y|-1)$ linking residual entropy to unavoidable error.
:::
...
- **Applying Fano without checking the alphabet.** The bound involves $\log_2(|\mathcal Y|-1)$; for a binary $Y$ that term vanishes and Fano reduces to $H(Y|X)\le h_2(P_e)$, which is much sharper than the generic form.
