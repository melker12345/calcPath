...
It measures how aligned two vectors are. Zero dot product means perpendicular.
:::
...
Lines and planes are then written with these two products:

$$\mathbf{r}(t) = \mathbf{r}_0 + t\mathbf{v}, \qquad \mathbf{n} \cdot (\mathbf{r} - \mathbf{r}_0) = 0.$$

A line is a point plus a direction; a plane is every displacement from $\mathbf{r}_0$ that is perpendicular to the normal vector $\mathbf{n}$.
...
:::solution
1. The plane equation is $\mathbf{n} \cdot (\mathbf{r} - \mathbf{r}_0) = 0$.
2. $2(x - 1) + (-1)(y - 2) + 4(z - 3) = 0$.
3. Expand: $2x - 2 - y + 2 + 4z - 12 = 0$.
4. Simplify: $2x - y + 4z = 12$.
:::
...
:::definition[Vector function and velocity]{#vector-function}
A vector function $\mathbf{r}(t) = \langle x(t), y(t), z(t) \rangle$ traces a curve in 3D space as the parameter $t$ varies. Its derivative

$$\mathbf{r}'(t) = \langle x'(t), y'(t), z'(t) \rangle$$

is the velocity vector — it points in the direction of motion and its magnitude is the speed.
:::

Think of $t$ as time: at each moment, the point $(x(t), y(t), z(t))$ is the particle's position.
...
Curvature $\kappa$ measures how sharply the curve turns:

$$\kappa = \frac{|\mathbf{T}'(t)|}{|\mathbf{r}'(t)|} = \frac{|\mathbf{r}' \times \mathbf{r}''|}{|\mathbf{r}'|^3}.$$

A straight line has $\kappa = 0$; a circle of radius $r$ has $\kappa = 1/r$ (tighter circle = higher curvature).
:::
...
Geometrically: $f_x(a, b)$ is the slope of the surface $z = f(x, y)$ in the $x$-direction at the point $(a, b)$. Slice the surface with the plane $y = b$ and you are back to a one-variable derivative.

Differentiating twice gives the higher-order partials $f_{xx}$, $f_{yy}$, $f_{xy}$, $f_{yx}$ — and the last two are not really different.
...
:::example[Showing a multivariable limit does not exist]
Investigate $\lim_{(x,y)\to(0,0)} \frac{xy}{x^2 + y^2}$.
:::

:::solution
1. Along the path $y = 0$: $\frac{x \cdot 0}{x^2 + 0} = 0$. Limit is $0$.
2. Along the path $y = x$: $\frac{x \cdot x}{x^2 + x^2} = \frac{x^2}{2x^2} = \frac{1}{2}$. Limit is $\frac{1}{2}$.
3. Two paths give different limits ($0 \neq \frac{1}{2}$), so the limit does not exist.
:::
...
It encodes all the directional information about how $f$ changes. Its direction points toward the steepest uphill, and its magnitude $|\nabla f|$ is the rate of steepest ascent.
:::

:::theorem[Directional derivative]{#directional-derivative}
...
This is the multivariable analog of the tangent line — the best linear approximation to the surface at that point.
...
:::theorem[Second derivative test for two variables]{#second-derivative-test-2d}
At a critical point compute the discriminant

$$D = f_{xx} f_{yy} - (f_{xy})^2.$$

If $D > 0$ and $f_{xx} > 0$: local min. If $D > 0$ and $f_{xx} < 0$: local max. If $D < 0$: saddle point. If $D = 0$: inconclusive.
:::

Lagrange multipliers solve constrained optimization: maximize/minimize $f(x,y)$ subject to a constraint $g(x,y) = c$.

:::recipe[Lagrange multipliers]{#lagrange-multipliers}
Solve

$$\nabla f = \lambda \nabla g \qquad \text{and} \qquad g(x,y) = c$$

simultaneously, then compare the values of $f$ at the solutions. The scalar $\lambda$ is the Lagrange multiplier.
:::
...
:::example[Lagrange multipliers: maximizing on a circle]
...
3. From equation 1: $\lambda = \frac{1}{2x}$. From equation 2: $\lambda = \frac{1}{y}$.
4. So $\frac{1}{2x} = \frac{1}{y} \Rightarrow y = 2x$.
5. Substitute into the constraint: $x^2 + (2x)^2 = 5 \Rightarrow 5x^2 = 5 \Rightarrow x = \pm 1$.
6. Points: $(1, 2)$ and $(-1, -2)$. $f(1,2) = 5$ (max) and $f(-1,-2) = -5$ (min).
:::
...
:::definition[Double integral]{#double-integral}
A double integral $\iint_R f(x,y)\,dA$ computes the volume under $z = f(x,y)$ over a region $R$ in the $xy$-plane. It is evaluated as an iterated integral,

$$\iint_R f\,dA = \int_a^b \int_{g_1(x)}^{g_2(x)} f(x,y)\,dy\,dx,$$

with the inner integral done first, treating the outer variable as a constant.
:::

:::theorem[Fubini's Theorem]{#fubini}
If $f$ is continuous on $R$, you can integrate in either order:

$$\iint f\,dy\,dx = \iint f\,dx\,dy.$$
:::

Sometimes one order is much easier — or the only one that can be done in closed form — so it is always worth writing both before choosing.
...
:::solution
1. Convert to polar: $x^2 + y^2 = r^2$, $dA = r\,dr\,d\theta$, and the disk becomes $0 \leq r \leq 2$, $0 \leq \theta \leq 2\pi$.
2. $\int_0^{2\pi} \int_0^2 e^{-r^2} \cdot r\,dr\,d\theta$.
3. The $\theta$ integral is trivial: $\int_0^{2\pi} d\theta = 2\pi$.
4. For the $r$ integral: $\int_0^2 r e^{-r^2}\,dr$. Let $u = -r^2$, $du = -2r\,dr$: $-\frac{1}{2}\int_0^{-4} e^u\,du = -\frac{1}{2}[e^{-4} - 1] = \frac{1-e^{-4}}{2}$.
5. Total: $2\pi \cdot \frac{1 - e^{-4}}{2} = \pi(1 - e^{-4})$.
:::
...
:::definition[Conservative field]{#conservative-field}
A vector field is conservative if $\mathbf{F} = \nabla f$ for some scalar function $f$, called the potential function.
:::

This means the field is the gradient of something. Conservative fields have a crucial property: the line integral between two points is independent of the path taken — only the endpoints matter.

:::theorem[Cross-partial test]{#cross-partial-test}
On a simply connected domain (no holes), $\mathbf{F} = \langle P, Q \rangle$ is conservative if and only if

$$\frac{\partial P}{\partial y} = \frac{\partial Q}{\partial x}.$$
:::

:::recipe[Finding the potential function]{#finding-potential}
If $\mathbf{F} = \langle P, Q \rangle$ passes the cross-partial test, find $f$ by integrating $f(x,y) = \int P\,dx$ (treating $y$ as constant), then determine any leftover function of $y$ by matching $f_y = Q$.
:::
...
:::definition[Scalar and vector line integrals]{#line-integrals}
$$\int_C f\,ds = \int_a^b f(\mathbf{r}(t)) |\mathbf{r}'(t)|\,dt, \qquad \int_C \mathbf{F} \cdot d\mathbf{r} = \int_a^b \mathbf{F}(\mathbf{r}(t)) \cdot \mathbf{r}'(t)\,dt.$$

In the first, $ds = |\mathbf{r}'(t)|\,dt$ is the arc length element; the second is the work integral.
:::

The scalar line integral computes the 'weighted length' of the curve — for example, the mass of a wire with density $f$ at each point, or the total cost of a path where $f$ is the cost per unit length.

In the vector line integral the dot product picks out the component of $\mathbf{F}$ in the direction of motion, so the integral computes the work done by the force field $\mathbf{F}$ on a particle moving along $C$. Force components perpendicular to the path contribute zero work.
...
This is the multivariable analog of $\int_a^b F'(x)\,dx = F(b) - F(a)$.

:::corollary[Closed loops in a conservative field]{#closed-loop-zero}
For a conservative field, the line integral around any closed loop is zero:

$$\oint_C \nabla f \cdot d\mathbf{r} = 0.$$
:::

You end where you started, so the potential difference is zero.
...
:::example[Work integral using the Fundamental Theorem]
Compute $\int_C \mathbf{F} \cdot d\mathbf{r}$ where $\mathbf{F} = \langle 2x, 2y \rangle$ and $C$ is any path from $(0,0)$ to $(3,4)$.
:::

:::solution
1. Check if $\mathbf{F}$ is conservative: $P = 2x$, $Q = 2y$. $P_y = 0$ and $Q_x = 0$. Since $P_y = Q_x$, $\mathbf{F}$ is conservative.
2. Find the potential function: $f_x = 2x \Rightarrow f = x^2 + g(y)$. Then $f_y = g'(y) = 2y \Rightarrow g(y) = y^2 + C$.
3. So $f(x,y) = x^2 + y^2$.
4. By the Fundamental Theorem: $\int_C \nabla f \cdot d\mathbf{r} = f(3,4) - f(0,0) = (9+16) - 0 = 25$.
5. The answer is $25$, regardless of which path we take from $(0,0)$ to $(3,4)$.
:::
...
Curl measures the 'rotation' or 'circulation' of a field:
...
A line integral around the boundary of a surface equals the surface integral of the curl.
:::

:::theorem[Divergence Theorem]{#divergence-theorem}
For a solid $E$ bounded by the closed surface $S$,

$$\oiint_S \mathbf{F} \cdot d\mathbf{S} = \iiint_E (\nabla \cdot \mathbf{F})\,dV.$$

The flux through a closed surface equals the triple integral of the divergence inside.
:::
...
:::example[Using Green's Theorem to compute area]
The area of a region $D$ can be written as $A = \frac12 \oint_C -y\,dx + x\,dy$.
:::

:::solution
1. This follows directly from Green's Theorem with $P = -y/2$, $Q = x/2$: $\frac{\partial Q}{\partial x} - \frac{\partial P}{\partial y} = 1$.
2. For the unit circle parametrized by $x=\cos t$, $y=\sin t$, $0 \le t \le 2\pi$, the line integral evaluates to $\pi$.
:::

## Deep dive: Conservative fields and path independence

<!-- section: conservative -->

:::theorem[Path independence]{#path-independence}
A vector field $\mathbf{F}$ is conservative — there exists a scalar potential $f$ with $\mathbf{F} = \nabla f$ — precisely when the line integral between two points depends only on the endpoints, not the path taken. On a simply connected domain this happens exactly when

$$\nabla \times \mathbf{F} = \mathbf{0}$$

everywhere (in 2D: $P_y = Q_x$).
:::

:::pitfall
If the domain is not simply connected (has holes), a field can have zero curl yet still not be conservative. The classic example is $\mathbf{F} = \langle -y/r^2, x/r^2 \rangle$ around the origin.
:::
