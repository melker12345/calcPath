...
**ELI5**: A vector function describes a path through space, like the GPS track of a drone. The derivative is the velocity — how fast and which direction it's flying at each moment. Speed is how fast; the unit tangent vector is which direction.
...
**Worked Example: Showing a multivariable limit does not exist**
...
**Worked Example: Lagrange multipliers: maximizing on a circle**
...
**Worked Example: Double integral in polar coordinates**
...
**ELI5**: A vector field is like a weather map with wind arrows at every location. At each point, the arrow tells you which way the wind blows and how strong it is.
...
**ELI5**: A scalar line integral is like measuring the total cost of a road trip, where different stretches of road have different tolls per mile. You multiply the toll rate by the distance at each point and add up along the route.
...
**Worked Example: Work integral using the Fundamental Theorem**
...
**Worked Example: Using Green's Theorem to compute area**
