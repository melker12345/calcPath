...
:::solution
1. Solve for the trig functions: $\cos t = x/2$, $\sin t = y/3$.
2. Use the identity $\cos^2 t + \sin^2 t = 1$: $(x/2)^2 + (y/3)^2 = 1$.
3. This is the equation of an ellipse centered at the origin with semi-major axis 3 (along y) and semi-minor axis 2 (along x).
4. The parametric equations also tell us the curve is traced counterclockwise starting from (2,0) when t=0.
:::
...
Provided $x'(t) \neq 0$, the slope of the tangent line to a parametric curve is

$$\frac{dy}{dx} = \frac{dy/dt}{dx/dt} = \frac{y'(t)}{x'(t)}.$$
:::

:::proof
This is the chain rule in disguise. If $y$ is a function of $t$ and $t$ is a function of $x$ (implicitly), then $\frac{dy}{dx} = \frac{dy}{dt} \cdot \frac{dt}{dx} = \frac{y'(t)}{x'(t)}$.
:::

The second derivative is more subtle, because you must differentiate the slope with respect to $t$ and then convert back:

$$\frac{d^2 y}{dx^2} = \frac{d}{dt} \left( \frac{dy}{dx} \right) \bigg/ \frac{dx}{dt} = \frac{d}{dt} \left( \frac{y'(t)}{x'(t)} \right) \cdot \frac{1}{x'(t)}.$$
...
:::solution
1. Compute derivatives: $x'(t) = 2t$, $y'(t) = 3t^2$.
2. Slope: $\frac{dy}{dx} = \frac{3t^2}{2t} = \frac{3t}{2}$ (for $t \neq 0$).
3. At $t=1$: slope = $\frac{3}{2}$.
4. Point: $x=1^2=1$, $y=1^3=1$, so the point is $(1,1)$.
5. Tangent line: $y - 1 = \frac{3}{2}(x - 1)$, or $y = \frac{3}{2}x - \frac{1}{2}$.
:::
...
The arc length of a parametric curve $x=x(t)$, $y=y(t)$ from $t=a$ to $t=b$ is
...
:::pitfall
The formula uses the *speed*, which is always positive. If the particle reverses direction, the arc length still accumulates positively. You do not get cancellation.
:::
...
:::solution
1. Derivatives: $x' = 3t^2$, $y' = 3t$.
2. $L = \int_0^1 \sqrt{(3t^2)^2 + (3t)^2}\,dt = \int_0^1 \sqrt{9t^4 + 9t^2}\,dt = \int_0^1 3t \sqrt{t^2 + 1}\,dt$.
3. Substitute $u = t^2 + 1$, $du = 2t\,dt$, so $\frac{3}{2} du = 3t\,dt$.
4. When $t=0$, $u=1$; when $t=1$, $u=2$.
5. $L = \frac{3}{2} \int_1^2 \sqrt{u}\,du = \frac{3}{2} \cdot \frac{2}{3} [u^{3/2}]_1^2 = [u^{3/2}]_1^2 = 2^{3/2} - 1 = 2\sqrt{2} - 1$.
:::
...
Assuming the curve is traversed left to right ($x'$ positive), the area under it (above the $x$-axis) from $t=a$ to $t=b$ is
...
For a closed curve traversed counterclockwise, the area enclosed is

$$A = \frac{1}{2} \int_a^b (x y' - y x')\,dt.$$
:::

That last formula is Green's theorem in disguise, and it is the one that makes cycloid-like curves tractable.
...
:::example[Area under a parametric curve]
Find the area under one arch of the cycloid $x = a(t - \sin t)$, $y = a(1 - \cos t)$, $0 \le t \le 2\pi$.
:::

:::solution
1. Here $x' = a(1 - \cos t)$, which is $\ge 0$ on $[0, 2\pi]$.
2. $A = \int_0^{2\pi} y(t) \cdot x'(t)\,dt = \int_0^{2\pi} a(1-\cos t) \cdot a(1 - \cos t)\,dt = a^2 \int_0^{2\pi} (1 - \cos t)^2\,dt$.
3. Expand: $(1 - 2\cos t + \cos^2 t)$. Use $\cos^2 t = \frac{1 + \cos 2t}{2}$.
4. Integral becomes $a^2 \int_0^{2\pi} \left( \frac{3}{2} - 2\cos t + \frac{1}{2}\cos 2t \right) dt = a^2 \left[ \frac{3}{2}t - 2\sin t + \frac{1}{4}\sin 2t \right]_0^{2\pi} = a^2 \cdot 3\pi$.
5. The area under one arch is $3\pi a^2$.
:::
...
:::notation
The conversion formulas run in both directions:

$$x = r \cos \theta, \quad y = r \sin \theta, \qquad r = \sqrt{x^2 + y^2}, \quad \theta = \tan^{-1}(y/x)$$

(the last with quadrant adjustment).
:::
...
The area swept out by a polar curve $r = f(\theta)$ from $\theta = \alpha$ to $\theta = \beta$ is

$$A = \frac{1}{2} \int_\alpha^\beta [f(\theta)]^2\,d\theta.$$
:::

:::proof
A tiny angle $d\theta$ creates a thin sector. The area of a sector of a circle is $\frac{1}{2} r^2 \Delta \theta$. Replacing $r$ with $f(\theta)$ and $d\theta$ for $\Delta \theta$ and integrating gives the formula.
:::
...
:::solution
1. The cardioid is traced once as $\theta$ goes from 0 to $2\pi$.
2. $A = \frac{1}{2} \int_0^{2\pi} [2(1 + \cos \theta)]^2 d\theta = \frac{1}{2} \int_0^{2\pi} 4(1 + 2\cos \theta + \cos^2 \theta) d\theta = 2 \int_0^{2\pi} (1 + 2\cos \theta + \frac{1+\cos 2\theta}{2}) d\theta$.
3. Simplify integrand: $1 + 2\cos \theta + \frac{1}{2} + \frac{1}{2}\cos 2\theta = \frac{3}{2} + 2\cos \theta + \frac{1}{2}\cos 2\theta$.
4. Integral: $2 \left[ \frac{3}{2}\theta + 2\sin \theta + \frac{1}{4}\sin 2\theta \right]_0^{2\pi} = 2 \cdot \frac{3}{2} \cdot 2\pi = 6\pi$.
5. Area inside the cardioid is $6\pi$.
:::
...
The area between two polar curves $r = f(\theta)$ (outer) and $r = g(\theta)$ (inner) from $\alpha$ to $\beta$ is
...
:::example[Area between two polar curves]
Find the area inside the cardioid $r = 2(1 + \cos \theta)$ and outside the circle $r = 2$.
:::

:::solution
1. Find the intersections: $2(1 + \cos \theta) = 2 \Rightarrow \cos \theta = 0 \Rightarrow \theta = \pm \tfrac{\pi}{2}$.
2. Test which curve is outer between them: at $\theta = 0$ the cardioid has $r = 4$ while the circle has $r = 2$, so the cardioid is the outer curve on $(-\tfrac{\pi}{2}, \tfrac{\pi}{2})$.
3. Set up the area with the cardioid as outer and the circle as inner: $A = \frac{1}{2} \int_{-\pi/2}^{\pi/2} \left[(2 + 2\cos \theta)^2 - 2^2\right] d\theta$.
4. Expand the integrand: $(2 + 2\cos\theta)^2 - 4 = 8\cos\theta + 4\cos^2\theta = 8\cos\theta + 2(1 + \cos 2\theta)$.
5. Integrate: $A = \frac{1}{2}\left[\,8\sin\theta + 2\theta + \sin 2\theta\,\right]_{-\pi/2}^{\pi/2} = \frac{1}{2}\big[(8 + \pi) - (-8 - \pi)\big] = \frac{1}{2}(16 + 2\pi)$.
6. The area is $8 + \pi$.
:::
...
The arc length of a polar curve $r = f(\theta)$ from $\theta = \alpha$ to $\theta = \beta$ is

$$L = \int_\alpha^\beta \sqrt{ r^2 + \left( \frac{dr}{d\theta} \right)^2 }\,d\theta.$$
:::

:::proof
Convert to parametric form with parameter $\theta$: $x = r\cos \theta$, $y = r\sin \theta$. Then compute $dx$ and $dy$, square, add, take square root. After simplification using $r^2 = x^2 + y^2$ you arrive at the formula.
:::
...
:::proposition[Tangent slope in polar coordinates]{#polar-slope}
To find the slope of a tangent line to $r = f(\theta)$, convert to parametric form with parameter $\theta$ and use $\frac{dy}{dx} = \frac{dy/d\theta}{dx/d\theta}$. Explicitly, with $x = r \cos \theta$ and $y = r \sin \theta$,
...
All conic sections (ellipses, parabolas, hyperbolas) can be written in the unified polar form

$$r = \frac{ed}{1 - e \cos \theta}$$

(or with $\sin$, depending on orientation), where $e$ is the eccentricity and $d$ is the directrix distance.
:::

The eccentricity alone names the curve: if $e < 1$ the conic is an ellipse; $e=1$ parabola; $e>1$ hyperbola.
