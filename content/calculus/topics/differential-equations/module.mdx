...
A DE is **linear** if $y$ and its derivatives appear only to the first power and are not multiplied together (e.g. $y'' + 4y = 0$); otherwise it is **nonlinear** (e.g. $y' = y^2$ or $y' = \sin y$). Linear equations have a far more complete solution theory, which is why we focus on them first.
...
The logistic equation $\frac{dy}{dt} = ky\left(1 - \frac{y}{M}\right)$ models growth that is limited by a carrying capacity $M$. When $y$ is small the factor $(1 - y/M) \approx 1$ and growth is nearly exponential; as $y \to M$ the factor shrinks toward $0$ and growth stalls.

It is separable, and its solution is $y(t) = \dfrac{M}{1 + C e^{-kt}}$, where $C$ is fixed by the initial population.
...
A Bernoulli equation has the form $y' + P(x)y = Q(x)y^n$ with $n \neq 0, 1$ (those two cases are already linear). The nonlinear $y^n$ term blocks the integrating-factor method directly.

The trick is the substitution $v = y^{1-n}$. Then $v' = (1-n)y^{-n}y'$, and dividing the equation by $y^n$ converts it into the linear first-order equation $v' + (1-n)P(x)v = (1-n)Q(x)$, which you solve with an integrating factor and then back-substitute.
...
For a non-homogeneous equation $y'' + p y' + q y = g(x)$, the general solution splits into two pieces: $y = y_h + y_p$, where $y_h$ is the general solution of the associated homogeneous equation and $y_p$ is any single particular solution.

Method of undetermined coefficients: guess a $y_p$ of the same form as $g(x)$ — a polynomial for a polynomial $g$, $Ae^{rx}$ for an exponential, $A\cos(\omega x) + B\sin(\omega x)$ for sines/cosines — then substitute to solve for the coefficients. If your guess already solves the homogeneous equation, multiply it by $x$ (or $x^2$) first.
