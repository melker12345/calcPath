...
**ELI5**: Real populations don't grow forever — food and space run out. Logistic growth starts off looking exponential, but as the population nears the environment's limit, growth slows and finally flattens at the carrying capacity.
...
**ELI5**: A Bernoulli equation is almost linear except for an annoying power of $y$. A clever change of variable absorbs that power and turns the problem into an ordinary linear equation you already know how to solve.
...
**ELI5**: First solve the equation as if the right-hand side were zero (that's the natural behavior of the system). Then find one solution that handles the driving term $g(x)$, and add the two together.
