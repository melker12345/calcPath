...
A critical point is a candidate for a local maximum or minimum, nothing more. What settles the question is the sign of $f'$ on either side.

:::theorem[First derivative test]{#first-derivative-test}
Let $c$ be a critical point of $f$ and examine the sign of $f'(x)$ on either side of it.

If $f'$ changes from $+$ to $-$ at $c$, then $f(c)$ is a local maximum. If $f'$ changes from $-$ to $+$ at $c$, then $f(c)$ is a local minimum. If $f'$ does not change sign (e.g., $f(x) = x^3$ at $x=0$), the critical point is neither a max nor a min.
:::

To apply the test: find all critical points, then build a sign chart for $f'(x)$ using test values in each interval.

:::intuition
Critical points are where the slope is zero (flat) or undefined (corner/cusp). The first derivative test is like checking the 'wind direction' before and after the point to see if you crested a hill or hit a valley.
:::
...
:::solution
1. find $f'(x)$: $f'(x) = 3x^2 - 3 = 3(x^2 - 1) = 3(x-1)(x+1)$.
2. set $f'(x) = 0$: $x = -1$ and $x = 1$ are the critical points.
3. build a sign chart. Test values in each interval:
4. On $(-\infty, -1)$, try $x = -2$: $f'(-2) = 3(4-1) = 9 > 0$. Increasing.
5. On $(-1, 1)$, try $x = 0$: $f'(0) = 3(0-1) = -3 < 0$. Decreasing.
6. On $(1, \infty)$, try $x = 2$: $f'(2) = 3(4-1) = 9 > 0$. Increasing.
7. At $x = -1$: $f'$ changes from $+$ to $-$, so this is a local maximum. $f(-1) = -1+3+1 = 3$.
8. At $x = 1$: $f'$ changes from $-$ to $+$, so this is a local minimum. $f(1) = 1-3+1 = -1$.
:::
...
The second derivative test is an alternative to the first derivative test that avoids building a sign chart.

:::theorem[Second derivative test]{#second-derivative-test}
Let $c$ be a critical point with $f'(c) = 0$.

If $f''(c) > 0$: the curve is concave up (bowl-shaped) at $c$, so $f(c)$ is a local minimum. If $f''(c) < 0$: the curve is concave down (arch-shaped) at $c$, so $f(c)$ is a local maximum. If $f''(c) = 0$: the test is inconclusive.
:::

When $f''(c) > 0$ the function is curving upward — any nearby point is higher. When $f''(c) < 0$ it is curving downward — any nearby point is lower.

The inconclusive case really is inconclusive: the critical point could be a min, max, or neither, and you must fall back to the first derivative test. Example: $f(x) = x^4$ at $x=0$ has $f''(0)=0$ but it's still a minimum. Meanwhile $f(x)=x^3$ at $x=0$ has $f''(0)=0$ and it's neither.
...
:::example[Classifying critical points with the second derivative test]
Classify the critical points of $f(x) = x^4 - 4x^3 + 6$.
:::

:::solution
1. $f'(x) = 4x^3 - 12x^2 = 4x^2(x - 3)$. Critical points: $x = 0$ and $x = 3$.
2. $f''(x) = 12x^2 - 24x = 12x(x-2)$.
3. At $x = 3$: $f''(3) = 12(3)(1) = 36 > 0$. Concave up → local minimum. $f(3) = 81 - 108 + 6 = -21$.
4. At $x = 0$: $f''(0) = 0$. Inconclusive! Must use the first derivative test.
5. Sign chart for $f'$ near $x=0$: $f'(-1) = 4(1)(-4) = -16 < 0$ and $f'(1) = 4(1)(-2) = -8 < 0$. No sign change → $x=0$ is neither a max nor a min (it's a flat inflection-like point).
:::
...
:::definition[Concavity]{#concavity}
...
If $f$ is continuous on a closed interval $[a,b]$, then $f$ attains an absolute maximum and an absolute minimum somewhere on $[a,b]$.
:::

Both 'continuous' and 'closed interval' are required — remove either condition and the theorem can fail.

The theorem promises the extrema exist; the closed interval method finds them.

:::recipe[The closed interval method]{#closed-interval-method}
1. Find all critical points of $f$ in the open interval $(a,b)$ — where $f'(x)=0$ or $f'$ is undefined.
2. Evaluate $f$ at each critical point and at both endpoints $a$ and $b$. Build a table of $(x, f(x))$ values.
3. The largest $f$-value in the table is the absolute maximum, the smallest is the absolute minimum.
:::
...
:::example[Finding absolute extrema on a closed interval]
Find the absolute max and min of $f(x) = 2x^3 - 3x^2 - 12x + 5$ on $[-2, 3]$.
:::

:::solution
1. $f'(x) = 6x^2 - 6x - 12 = 6(x^2 - x - 2) = 6(x-2)(x+1)$.
2. Critical points in $(-2, 3)$: $x = -1$ and $x = 2$.
3. Evaluate $f$ at all candidates: $f(-2) = -16-12+24+5 = 1$, $f(-1) = -2-3+12+5 = 12$, $f(2) = 16-12-24+5 = -15$, $f(3) = 54-27-36+5 = -4$.
4. Absolute maximum: $f(-1) = 12$. Absolute minimum: $f(2) = -15$.
5. Notice: the absolute minimum occurred at an interior critical point, not an endpoint. You must check all candidates.
:::
...
:::recipe[Sketching a curve]{#curve-sketching}
...
:::example[Full curve sketch of a cubic]
Sketch $f(x) = x^3 - 3x$.
:::

:::solution
1. Domain: all reals. Intercepts: $f(0) = 0$ ($y$-intercept). $x^3 - 3x = x(x^2-3) = 0$ gives $x = 0, \pm\sqrt{3}$.
2. Symmetry: $f(-x) = -x^3 + 3x = -(x^3-3x) = -f(x)$. The function is odd — symmetric about the origin.
3. $f'(x) = 3x^2 - 3 = 3(x-1)(x+1)$. Critical points: $x = -1$ and $x = 1$.
4. Sign chart for $f'$: positive on $(-\infty,-1)$, negative on $(-1,1)$, positive on $(1,\infty)$. So $f$ increases, then decreases, then increases.
5. $f(-1) = 2$ (local max), $f(1) = -2$ (local min).
6. $f''(x) = 6x$. Zero at $x = 0$. Negative for $x<0$ (concave down), positive for $x>0$ (concave up). Inflection point at $(0, 0)$.
7. Shape: rises to local max $(-1, 2)$, curves concave-down through $(0,0)$, falls to local min $(1, -2)$, then rises — the classic S-shaped cubic.
:::
...
Every optimization word problem is solved by the same six moves.

:::recipe[Solving an optimization problem]{#optimization-method}
1. Draw a diagram and define variables. Label everything.
2. Write the objective function (what you want to maximize or minimize).
3. Write the constraint equation (the relationship that limits your variables).
4. Use the constraint to eliminate one variable from the objective, getting a function of a single variable.
5. Find the critical points of this function. Test them (and the endpoints, if the domain is closed) to identify the optimum.
6. Answer the original question. Make sure you're solving for what was asked.
:::

Common setups: maximize area given a perimeter constraint, minimize material given a volume constraint, maximize revenue.

:::intuition
Turn a word problem into a function of one variable, then use calculus to find its peak or valley. The answer is usually the 'just right' size or price.
:::

:::example[Optimization: minimizing material for a box]
Design an open-top box with volume $32\text{ cm}^3$ using the least material. The base is square.
:::

:::solution
1. define variables: let $x$ = side length of the square base, $h$ = height.
2. objective function: surface area (what we minimize). Open top means: $S = x^2 + 4xh$ (base + 4 sides, no top).
3. constraint: volume $= x^2 h = 32$, so $h = \frac{32}{x^2}$.
4. substitute to get one variable: $S(x) = x^2 + 4x \cdot \frac{32}{x^2} = x^2 + \frac{128}{x}$.
5. differentiate: $S'(x) = 2x - \frac{128}{x^2}$.
6. Set $S'(x) = 0$: $2x = \frac{128}{x^2}$, so $2x^3 = 128$, giving $x^3 = 64$, thus $x = 4$.
7. Then $h = \frac{32}{16} = 2$.
8. verify minimum: $S''(x) = 2 + \frac{256}{x^3}$. At $x=4$: $S''(4) = 2 + 4 = 6 > 0$. Confirmed minimum.
9. Minimum surface area: $S(4) = 16 + 32 = 48\text{ cm}^2$.
:::
...
:::recipe[Solving a related-rates problem]{#related-rates-method}
1. Draw a picture and identify all variables. Label which quantities are changing.
2. Write an equation relating the variables (e.g., Pythagorean theorem, area formula, volume formula).
3. Differentiate both sides with respect to time $t$ using implicit differentiation.
4. Plug in all known values and rates at the specific instant in question.
5. Solve for the unknown rate.
:::

:::pitfall
Never plug in specific values before differentiating. The equation must remain general during differentiation because the variables are changing.
:::
...
:::example[Related rates: a sliding ladder]
...
:::solution
1. draw and label: $x$ = distance from base to wall, $y$ = height of top on wall. Both change with time $t$.
2. equation relating variables: by the Pythagorean theorem, $x^2 + y^2 = 100$ (the ladder is always 10 ft).
3. differentiate with respect to $t$: $2x\frac{dx}{dt} + 2y\frac{dy}{dt} = 0$.
4. Note: we did NOT plug in numbers yet. This is critical — both $x$ and $y$ are changing.
5. plug in the known values at the instant in question: $x = 6$, $\frac{dx}{dt} = 1$. Find $y$: $y = \sqrt{100 - 36} = 8$.
6. solve: $2(6)(1) + 2(8)\frac{dy}{dt} = 0$, so $12 + 16\frac{dy}{dt} = 0$, giving $\frac{dy}{dt} = -\frac{3}{4}$.
7. The top slides down at $\frac{3}{4}$ ft/s. The negative sign confirms the top is moving downward.
:::
...
:::definition[Velocity and acceleration]{#velocity-acceleration}
Given a position function $s(t)$, velocity and acceleration are

$$v(t) = s'(t), \qquad a(t) = v'(t) = s''(t).$$
:::

This is the most natural physical application of derivatives: the sign of $v$ says which way the particle is going, and $a$ says how the velocity itself is changing.
...
:::definition[Displacement and total distance]{#displacement-distance}
Over $[a,b]$, displacement is the net change in position and total distance travelled is the accumulated unsigned motion:

$$\text{displacement} = \int_a^b v(t)\,dt, \qquad \text{total distance} = \int_a^b |v(t)|\,dt.$$
:::

Displacement can be zero if the particle returns to where it started; total distance is always $\ge 0$. To compute total distance, find where $v(t) = 0$, split the integral, and flip signs on intervals where $v < 0$.
...
:::example[Analyzing particle motion]
A particle has position $s(t) = t^3 - 6t^2 + 9t$ for $t \geq 0$. Describe its motion.
:::

:::solution
1. $v(t) = s'(t) = 3t^2 - 12t + 9 = 3(t-1)(t-3)$.
2. $v(t) = 0$ at $t = 1$ and $t = 3$ — the particle stops at these times.
3. Sign chart: $v > 0$ on $(0,1)$ (moving right), $v < 0$ on $(1,3)$ (moving left), $v > 0$ on $(3,\infty)$ (moving right again).
4. The particle changes direction at $t = 1$ and $t = 3$.
5. $a(t) = 6t - 12 = 0$ at $t = 2$. For $t < 2$, $a < 0$; for $t > 2$, $a > 0$.
6. On $(0,1)$: $v > 0$, $a < 0$ — moving right but slowing down. On $(1,2)$: $v < 0$, $a < 0$ — moving left and speeding up. On $(2,3)$: $v < 0$, $a > 0$ — moving left but slowing down.
:::
...
This is the equation of the tangent line at $x = a$, repurposed as an approximation tool.
:::
...
:::example[Linearization to estimate a cube root]
Estimate $\sqrt[3]{8.1}$ using linearization.
:::

:::solution
1. Choose $f(x) = \sqrt[3]{x} = x^{1/3}$ and anchor at $a = 8$ (because $\sqrt[3]{8} = 2$ is exact).
2. $f'(x) = \frac{1}{3}x^{-2/3} = \frac{1}{3x^{2/3}}$. So $f'(8) = \frac{1}{3 \cdot 4} = \frac{1}{12}$.
3. Linearization: $L(x) = 2 + \frac{1}{12}(x - 8)$.
4. Estimate: $L(8.1) = 2 + \frac{1}{12}(0.1) = 2 + 0.00833 \approx 2.0083$.
5. Actual value: $\sqrt[3]{8.1} \approx 2.00830...$  The estimate is accurate to 4 decimal places!
:::
...
Start with an initial guess $x_0$ and iterate

$$x_{n+1} = x_n - \frac{f(x_n)}{f'(x_n)},$$

stopping when $|x_{n+1} - x_n| < \varepsilon$ for a desired tolerance $\varepsilon$, or when $|f(x_n)| < \varepsilon$.
:::
...
:::proposition[Error estimate for Newton's method]
If $r$ is the true root, then

$$|x_{n+1} - r| \approx \frac{|f''(r)|}{2|f'(r)|} \cdot |x_n - r|^2.$$

This quantifies the quadratic convergence: squaring a small error makes it very small indeed.
:::

:::pitfall
The method can fail if $f'(x_n) = 0$ (horizontal tangent), if the initial guess is too far from the root, or if the function oscillates. Cycling is possible — Newton's method applied to $x^3 - 2x + 2$ starting at $x_0 = 0$ enters an infinite loop between $1$ and $0$.
:::

:::example[Newton's method: finding $\sqrt{5}$]
We want to solve $f(x) = x^2 - 5 = 0$, so $f'(x) = 2x$.
:::

:::solution
1. Iteration formula: $x_{n+1} = x_n - \frac{x_n^2 - 5}{2x_n} = \frac{x_n + 5/x_n}{2}$.
2. Start with $x_0 = 2$: $x_1 = \frac{2 + 5/2}{2} = \frac{2 + 2.5}{2} = 2.25$.
3. $x_2 = \frac{2.25 + 5/2.25}{2} = \frac{2.25 + 2.2222}{2} = 2.23611$.
4. $x_3 = 2.23607$ (already accurate to 5 decimal places). $\sqrt{5} = 2.23607...$
5. In just 3 iterations we went from a rough guess to 5-digit accuracy — that's quadratic convergence in action.
:::
