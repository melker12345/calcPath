...
**Magnitude and the displacement between two points**

1. Two points are given: $P = (1, 2, 3)$ and $Q = (4, 6, 3)$. Find the displacement vector $\overrightarrow{PQ}$ and its length.
2. The displacement is "head minus tail," component by component: $\overrightarrow{PQ} = Q - P = \langle 4-1,\; 6-2,\; 3-3 \rangle = \langle 3, 4, 0 \rangle$. Subtract in this order because the arrow points *from* $P$ *to* $Q$.
3. Its magnitude is $\|\overrightarrow{PQ}\| = \sqrt{3^2 + 4^2 + 0^2} = \sqrt{9+16+0} = \sqrt{25} = 5$.
4. This length is exactly the straight-line distance between the points $P$ and $Q$ — the norm of a displacement vector is a distance.
5. Sanity check: the third components of $P$ and $Q$ are equal, so the two points lie at the same height and the third component of the displacement is $0$, as computed. The magnitude $5$ is nonnegative, as every magnitude must be. ✓
...
**Linear combinations, span, and independence.** Combining these two operations gives the single most important construction in linear algebra. A *linear combination* of vectors $\mathbf{v}_1, \ldots, \mathbf{v}_k$ is any vector of the form $c_1\mathbf{v}_1 + c_2\mathbf{v}_2 + \cdots + c_k\mathbf{v}_k$, where the coefficients $c_1, \ldots, c_k$ are scalars. (The decomposition $\langle a,b,c\rangle = a\mathbf{e}_1 + b\mathbf{e}_2 + c\mathbf{e}_3$ above is exactly such a combination.) The *span* of a set of vectors is the collection of *all* linear combinations you can build from them — for two non-parallel vectors in $\mathbb{R}^3$, the span is the plane through the origin that contains them.

A set $\{\mathbf{v}_1, \ldots, \mathbf{v}_k\}$ is *linearly independent* when the only linear combination that equals the zero vector is the trivial one: $c_1\mathbf{v}_1 + \cdots + c_k\mathbf{v}_k = \mathbf{0}$ forces $c_1 = \cdots = c_k = 0$. If instead some *nontrivial* choice of coefficients (not all zero) produces $\mathbf{0}$, the set is *linearly dependent* — and then at least one vector can be written as a linear combination of the others, so it adds nothing new to the span. Two vectors are dependent exactly when one is a scalar multiple of the other (i.e. they are parallel).
...
1. Are $\mathbf{u} = \langle 1,-1,2\rangle$, $\mathbf{v} = \langle 0,1,3\rangle$, and $\mathbf{w} = \langle 2,-1,7\rangle$ linearly dependent?
2. We ask whether some nontrivial combination gives $\mathbf{0}$ — equivalently, whether one vector is a linear combination of the others. Test $\mathbf{w} = a\mathbf{u} + b\mathbf{v}$ and solve for $a, b$.
3. Component by component: first gives $2 = a(1) + b(0) = a$, so $a = 2$. Third gives $7 = a(2) + b(3) = 4 + 3b$, so $b = 1$.
4. Check the remaining (second) component with $a=2, b=1$: $a(-1) + b(1) = -2 + 1 = -1$, which matches the second component of $\mathbf{w}$. ✓ All three components agree, so $\mathbf{w} = 2\mathbf{u} + \mathbf{v}$.
5. Because $\mathbf{w}$ is a linear combination of $\mathbf{u}$ and $\mathbf{v}$, the set is linearly **dependent**. Equivalently, $2\mathbf{u} + \mathbf{v} - \mathbf{w} = \mathbf{0}$ is a nontrivial combination equal to the zero vector, confirming dependence directly.
...
4. $\cos\theta = 8/(\sqrt{14}\cdot\sqrt{21}) = 8/\sqrt{294} \approx 0.466$. Here $\sqrt{14}\cdot\sqrt{21} = \sqrt{294}$ because $14 \cdot 21 = 294$.
5. $\theta = \arccos(0.466) \approx 62.2°$.
6. Sanity check: the dot product came out positive ($8 > 0$), which by the sign rule above predicts an acute angle — and indeed $62.2° < 90°$. ✓ Also $|\cos\theta| \approx 0.466 \le 1$, consistent with Cauchy-Schwarz. ✓
...
Vector projection: $\text{proj}_{\mathbf{v}}\mathbf{u} = \dfrac{\mathbf{u}\cdot\mathbf{v}}{\mathbf{v}\cdot\mathbf{v}}\,\mathbf{v} = \dfrac{\mathbf{u}\cdot\mathbf{v}}{\|\mathbf{v}\|^2}\,\mathbf{v}$. The result is a vector parallel to $\mathbf{v}$. This requires $\mathbf{v} \neq \mathbf{0}$: you cannot project onto the zero vector, since it defines no direction and the formula would divide by $\|\mathbf{v}\|^2 = 0$.
...
Magnitude: $\|\mathbf{u}\times\mathbf{v}\| = \|\mathbf{u}\|\,\|\mathbf{v}\|\sin\theta$, where $\theta \in [0,\pi]$ is the angle between the vectors, so $\sin\theta \ge 0$ and the magnitude is nonnegative. This equals the area of the parallelogram spanned by $\mathbf{u}$ and $\mathbf{v}$. If the vectors are parallel ($\theta = 0$ or $\pi$, so $\sin\theta = 0$), the cross product is $\mathbf{0}$.
