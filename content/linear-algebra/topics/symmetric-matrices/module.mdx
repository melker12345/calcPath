...
1. Find $Q$ and $\Lambda$ for $A=\begin{pmatrix}3&1\\1&3\end{pmatrix}$.
2. Characteristic equation: $(3-\lambda)^2-1=0\Rightarrow\lambda^2-6\lambda+8=0\Rightarrow\lambda_1=4,\lambda_2=2$.
3. For $\lambda_1=4$: $(A-4I)\mathbf{v}=0\Rightarrow\begin{pmatrix}-1&1\\1&-1\end{pmatrix}\mathbf{v}=0$. Eigenvector: $\langle 1,1\rangle$, normalised: $\mathbf{q}_1=\langle 1/\sqrt{2},1/\sqrt{2}\rangle$.
4. For $\lambda_2=2$: eigenvector $\langle 1,-1\rangle$, normalised: $\mathbf{q}_2=\langle 1/\sqrt{2},-1/\sqrt{2}\rangle$.
5. $Q=\begin{pmatrix}1/\sqrt{2}&1/\sqrt{2}\\1/\sqrt{2}&-1/\sqrt{2}\end{pmatrix}$, $\Lambda=\begin{pmatrix}4&0\\0&2\end{pmatrix}$. Verify orthogonality: $Q^TQ=I$ ✓ (columns are unit length and mutually perpendicular).
6. Sanity check the decomposition itself. $Q\Lambda=\begin{pmatrix}4/\sqrt2&2/\sqrt2\\4/\sqrt2&-2/\sqrt2\end{pmatrix}$; then $Q\Lambda Q^T=\begin{pmatrix}4/\sqrt2&2/\sqrt2\\4/\sqrt2&-2/\sqrt2\end{pmatrix}\begin{pmatrix}1/\sqrt2&1/\sqrt2\\1/\sqrt2&-1/\sqrt2\end{pmatrix}=\begin{pmatrix}(4+2)/2&(4-2)/2\\(4-2)/2&(4+2)/2\end{pmatrix}=\begin{pmatrix}3&1\\1&3\end{pmatrix}=A$ ✓.
...
**Classify and diagonalise a quadratic form**

1. Take $Q(x,y)=5x^2-4xy+5y^2$. First build the symmetric matrix. With the convention $Q=ax^2+2bxy+cy^2$, we read off $a=5$, $c=5$, and $2b=-4\Rightarrow b=-2$, so $A=\begin{pmatrix}5&-2\\-2&5\end{pmatrix}$.
2. Sylvester's criterion (fast classification): first minor $=5>0$; second minor $=\det A=5\cdot5-(-2)^2=25-4=21>0$. Both positive $\Rightarrow$ $Q$ is positive definite.
3. Confirm via eigenvalues: $\text{tr}=10$, $\det=21$, so $\lambda^2-10\lambda+21=0\Rightarrow(\lambda-3)(\lambda-7)=0$, giving $\lambda_1=3$, $\lambda_2=7$. Both positive $\Rightarrow$ positive definite, agreeing with step 2.
4. Principal axes. For $\lambda=3$: $(A-3I)\mathbf{v}=\begin{pmatrix}2&-2\\-2&2\end{pmatrix}\mathbf{v}=0\Rightarrow x=y$, eigenvector $\langle 1,1\rangle$, normalised $\mathbf{q}_1=\langle 1/\sqrt2,1/\sqrt2\rangle$. For $\lambda=7$: $\begin{pmatrix}-2&-2\\-2&-2\end{pmatrix}\mathbf{v}=0\Rightarrow x=-y$, eigenvector $\langle 1,-1\rangle$, normalised $\mathbf{q}_2=\langle 1/\sqrt2,-1/\sqrt2\rangle$.
5. In the eigenvector coordinates $\mathbf{x}=Q\mathbf{y}$ (with $Q=[\mathbf{q}_1\ \mathbf{q}_2]$) the cross term vanishes: $Q=3y_1^2+7y_2^2$.
6. Sanity check at $\mathbf{x}=\langle 1,1\rangle$: directly, $Q=5(1)-4(1)+5(1)=6$. In eigen-coordinates, $\mathbf{x}=\sqrt2\,\mathbf{q}_1$, so $y_1=\sqrt2$, $y_2=0$ and $3(\sqrt2)^2+7(0)^2=3\cdot2=6$ ✓.
...
1. Find the SVD of $A=\begin{pmatrix}2&2\\-1&1\end{pmatrix}$.
2. Form $A^TA=\begin{pmatrix}2&-1\\2&1\end{pmatrix}\begin{pmatrix}2&2\\-1&1\end{pmatrix}=\begin{pmatrix}5&3\\3&5\end{pmatrix}$.
3. Eigenvalues of $A^TA$: $\text{tr}=10$, $\det=25-9=16$, so $\lambda^2-10\lambda+16=0\Rightarrow(\lambda-8)(\lambda-2)=0$, giving $\lambda_1=8$, $\lambda_2=2$. Singular values $\sigma_1=\sqrt8=2\sqrt2$, $\sigma_2=\sqrt2$.
4. Right singular vectors (eigenvectors of $A^TA$). For $\lambda=8$: $(A^TA-8I)=\begin{pmatrix}-3&3\\3&-3\end{pmatrix}\Rightarrow x=y$, so $\mathbf{v}_1=\langle 1/\sqrt2,1/\sqrt2\rangle$. For $\lambda=2$: $x=-y$, so $\mathbf{v}_2=\langle 1/\sqrt2,-1/\sqrt2\rangle$.
5. Left singular vectors via $\mathbf{u}_i=A\mathbf{v}_i/\sigma_i$. $A\mathbf{v}_1=\frac{1}{\sqrt2}\begin{pmatrix}2+2\\-1+1\end{pmatrix}=\frac{1}{\sqrt2}\langle 4,0\rangle$, so $\mathbf{u}_1=\frac{1}{2\sqrt2}\cdot\frac{1}{\sqrt2}\langle 4,0\rangle=\langle 1,0\rangle$. $A\mathbf{v}_2=\frac{1}{\sqrt2}\begin{pmatrix}2-2\\-1-1\end{pmatrix}=\frac{1}{\sqrt2}\langle 0,-2\rangle$, so $\mathbf{u}_2=\frac{1}{\sqrt2}\cdot\frac{1}{\sqrt2}\langle 0,-2\rangle=\langle 0,-1\rangle$.
6. Assemble: $U=\begin{pmatrix}1&0\\0&-1\end{pmatrix}$, $\Sigma=\begin{pmatrix}2\sqrt2&0\\0&\sqrt2\end{pmatrix}$, $V=\begin{pmatrix}1/\sqrt2&1/\sqrt2\\1/\sqrt2&-1/\sqrt2\end{pmatrix}$.
7. Sanity check $U\Sigma V^T$: $U\Sigma=\begin{pmatrix}2\sqrt2&0\\0&-\sqrt2\end{pmatrix}$, and multiplying by $V^T=\begin{pmatrix}1/\sqrt2&1/\sqrt2\\1/\sqrt2&-1/\sqrt2\end{pmatrix}$ gives $\begin{pmatrix}2&2\\-1&1\end{pmatrix}=A$ ✓.
...
**PCA on a tiny dataset**

1. Data (three 2-D observations): $(2,1),(-1,-2),(-1,1)$. First **centre** it: the mean is $\left(\frac{2-1-1}{3},\frac{1-2+1}{3}\right)=(0,0)$, so the data is already centred (no shift needed).
2. Build the covariance matrix $C=\frac{1}{m-1}X^TX$ with $m=3$, so $m-1=2$. The needed sums over the three points are $\sum x^2=4+1+1=6$, $\sum y^2=1+4+1=6$, $\sum xy=(2)(1)+(-1)(-2)+(-1)(1)=2+2-1=3$. Hence $C=\frac{1}{2}\begin{pmatrix}6&3\\3&6\end{pmatrix}=\begin{pmatrix}3&1.5\\1.5&3\end{pmatrix}$.
3. Eigenvalues of $C$: $\text{tr}=6$, $\det=3\cdot3-1.5^2=9-2.25=6.75$, so $\lambda^2-6\lambda+6.75=0\Rightarrow\lambda=\frac{6\pm\sqrt{36-27}}{2}=\frac{6\pm3}{2}$, giving $\lambda_1=4.5$, $\lambda_2=1.5$. Both $\geq 0$, as required for a covariance matrix.
4. First principal component (eigenvector for $\lambda_1=4.5$): $(C-4.5I)\mathbf{v}=\begin{pmatrix}-1.5&1.5\\1.5&-1.5\end{pmatrix}\mathbf{v}=0\Rightarrow x=y$, so the direction of maximum variance is $\langle 1,1\rangle$, normalised $\mathbf{q}_1=\langle 1/\sqrt2,1/\sqrt2\rangle$.
5. Variance explained: the first component captures $\lambda_1$ of the total variance $\lambda_1+\lambda_2=4.5+1.5=6$. Fraction $=4.5/6=0.75$, i.e. projecting onto the single line through $\langle 1,1\rangle$ retains $75\%$ of the spread.
6. Sanity check: the total variance also equals $\text{tr}(C)=3+3=6$, matching $\lambda_1+\lambda_2$ ✓ (trace is invariant under the orthogonal change of basis PCA performs).
