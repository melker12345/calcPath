...
**Is $V=\{\langle x,y,z\rangle : x+y+z=0\}$ a subspace of $\mathbb{R}^3$?**

1. Check $\mathbf{0}\in V$: for $\langle 0,0,0\rangle$ we have $0+0+0=0$ ✓.
2. Check closure under addition: take $\mathbf{u}=\langle u_1,u_2,u_3\rangle$ and $\mathbf{v}=\langle v_1,v_2,v_3\rangle$ in $V$, so $u_1+u_2+u_3=0$ and $v_1+v_2+v_3=0$. Their sum satisfies $(u_1+v_1)+(u_2+v_2)+(u_3+v_3) = (u_1+u_2+u_3)+(v_1+v_2+v_3) = 0+0 = 0$, so $\mathbf{u}+\mathbf{v}\in V$ ✓.
3. Check closure under scaling: for $c\in\mathbb{R}$, $cu_1+cu_2+cu_3 = c(u_1+u_2+u_3) = c\cdot 0 = 0$, so $c\mathbf{u}\in V$ ✓.
4. All three conditions hold, so $V$ is a subspace. Sanity check: $V$ is a plane through the origin (a single homogeneous equation in $\mathbb{R}^3$), which is exactly the kind of flat-through-origin set the ELI5 describes.
5. Contrast: $W=\{\langle x,y,z\rangle : x+y+z=1\}$ is **not** a subspace — it fails step 1 since $0+0+0=0\neq 1$, so $\mathbf{0}\notin W$. That single failed check is enough to disqualify it.
...
1. Is $\mathbf{b}=\langle 1,5,3\rangle$ in $\text{span}\{\mathbf{v}_1,\mathbf{v}_2\}$ with $\mathbf{v}_1=\langle 1,1,1\rangle$, $\mathbf{v}_2=\langle 0,2,1\rangle$?
2. Row-reduce $[\mathbf{v}_1\;\mathbf{v}_2\;|\;\mathbf{b}]$: $\left(\begin{array}{cc|c}1&0&1\\1&2&5\\1&1&3\end{array}\right)\to\left(\begin{array}{cc|c}1&0&1\\0&1&2\\0&0&0\end{array}\right)$.
3. Consistent. $c_1=1$, $c_2=2$: $\mathbf{b} = 1\cdot\mathbf{v}_1+2\cdot\mathbf{v}_2$.
4. Check: $\langle 1,1,1\rangle+2\langle 0,2,1\rangle = \langle 1,5,3\rangle$ ✓.
...
**Dependence check in $\mathbb{R}^3$**

1. Are $\mathbf{v}_1=\langle 1,0,2\rangle$, $\mathbf{v}_2=\langle 0,1,3\rangle$, $\mathbf{v}_3=\langle 2,1,7\rangle$ independent?
2. Row-reduce: $\begin{pmatrix}1&0&2\\0&1&3\\2&1&7\end{pmatrix}\to\begin{pmatrix}1&0&2\\0&1&3\\0&0&0\end{pmatrix}$. Only 2 pivots — dependent.
3. From RREF: $\mathbf{v}_3=2\mathbf{v}_1+\mathbf{v}_2$. Check: $2\langle 1,0,2\rangle+\langle 0,1,3\rangle=\langle 2,1,7\rangle$ ✓.


## Basis and dimension {#basis}

A basis for a subspace $V$ is a set of vectors that is both linearly independent and spans $V$. It is simultaneously a minimal spanning set (drop any vector and it no longer spans $V$) and a maximal independent set (add any other vector of $V$ and it becomes dependent) — it sits exactly at the boundary where "enough to reach everything" meets "no redundancy."
...
**Find a basis and the dimension of $V=\{\langle x,y,z\rangle : x+y+z=0\}$.**

1. Solve the constraint for one variable in terms of the others. Treat $y=s$ and $z=t$ as free; then $x = -s-t$.
2. Write the general vector using the free variables: $\langle x,y,z\rangle = \langle -s-t,\; s,\; t\rangle = s\langle -1,1,0\rangle + t\langle -1,0,1\rangle$.
3. Every vector in $V$ is a combination of $\mathbf{b}_1=\langle -1,1,0\rangle$ and $\mathbf{b}_2=\langle -1,0,1\rangle$, so these two span $V$.
4. They are independent: neither is a scalar multiple of the other (their $y$ and $z$ entries can't both be matched by one constant). A spanning set of two independent vectors is a basis.
5. Therefore $\dim(V)=2$. Sanity check: verify each basis vector satisfies the constraint — $-1+1+0=0$ ✓ and $-1+0+1=0$ ✓. This matches the intuition that one equation removes one degree of freedom from $\mathbb{R}^3$, leaving $3-1=2$.
...
The row space $\text{Row}(A)=\text{Col}(A^T)$ has dimension $\text{rank}(A)$ as well. Keep the ambient spaces straight for an $m\times n$ matrix $A$: $\text{Row}(A)$ and $\text{Null}(A)$ live in the input space $\mathbb{R}^n$, while $\text{Col}(A)$ and $\text{Null}(A^T)$ live in the output space $\mathbb{R}^m$. The fundamental theorem of linear algebra says each pair splits its space into orthogonal complements: $\text{Row}(A)^\perp = \text{Null}(A)$ inside $\mathbb{R}^n$, and $\text{Col}(A)^\perp = \text{Null}(A^T)$ inside $\mathbb{R}^m$. "Orthogonal complements" means the two subspaces meet only at $\mathbf{0}$, every vector of one is perpendicular to every vector of the other, and their dimensions add up to the whole space.
...
**Find bases for $\text{Col}(A)$ and $\text{Null}(A)$, and verify rank-nullity, for**
$$A=\begin{pmatrix}1&2&3\\0&1&1\\1&3&4\end{pmatrix}.$$

1. Row-reduce. Subtract row 1 from row 3: $\begin{pmatrix}1&2&3\\0&1&1\\0&1&1\end{pmatrix}$. Subtract row 2 from row 3: $\begin{pmatrix}1&2&3\\0&1&1\\0&0&0\end{pmatrix}$. Clear above the second pivot with row 1 $\to$ row 1 $-2\cdot$ row 2: RREF $=\begin{pmatrix}1&0&1\\0&1&1\\0&0&0\end{pmatrix}$.
2. Pivots are in columns 1 and 2, so $\text{rank}(A)=2$. Basis for $\text{Col}(A)$: take the **original** columns at those pivot positions, $\{\langle 1,0,1\rangle,\ \langle 2,1,3\rangle\}$ — not the RREF columns.
3. Basis for $\text{Null}(A)$: column 3 is free, so set $x_3=t$. The RREF rows give $x_1+x_3=0$ and $x_2+x_3=0$, i.e. $x_1=-t$, $x_2=-t$. The general solution is $t\langle -1,-1,1\rangle$, so $\{\langle -1,-1,1\rangle\}$ is a basis and $\text{nullity}(A)=1$.
4. Check rank-nullity: $\text{rank}+\text{nullity}=2+1=3=n$ ✓ (the matrix has $3$ columns).
5. Sanity check the null vector: $A\langle -1,-1,1\rangle$ has entries $1(-1)+2(-1)+3(1)=0$, $0(-1)+1(-1)+1(1)=0$, $1(-1)+3(-1)+4(1)=0$, so $A\langle -1,-1,1\rangle=\mathbf{0}$ ✓.
