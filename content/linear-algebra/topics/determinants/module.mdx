...
1. $A = \begin{pmatrix}1&2&3\\0&4&5\\1&0&6\end{pmatrix}$. Expand along row 1 (start with the most zeros, but row 1 works fine here).
2. $\det(A) = 1\cdot\det\begin{pmatrix}4&5\\0&6\end{pmatrix} - 2\cdot\det\begin{pmatrix}0&5\\1&6\end{pmatrix} + 3\cdot\det\begin{pmatrix}0&4\\1&0\end{pmatrix}$.
3. $= 1(24-0) - 2(0-5) + 3(0-4)$.
4. $= 24 + 10 - 12 = 22$.
5. Sanity check: expand instead along column 1 (entries $1,0,1$). Only the two $1$'s contribute: $1\cdot(+1)\det\begin{pmatrix}4&5\\0&6\end{pmatrix} + 1\cdot(-1)^{3+1}\det\begin{pmatrix}2&3\\4&5\end{pmatrix} = 24 + (10-12) = 22$. Same answer from a different expansion. ✓

**Worked Example:**

**Triangular shortcut**

1. $B = \begin{pmatrix}2&7&-3\\0&5&1\\0&0&4\end{pmatrix}$ is upper triangular (all entries below the diagonal are $0$).
2. Rather than a full cofactor expansion, use the rule: the determinant is the product of the diagonal entries.
3. $\det(B) = 2\cdot 5\cdot 4 = 40$.
4. Why it works: expanding along column 1, only the top-left $2$ survives, and the remaining $2\times 2$ block $\begin{pmatrix}5&1\\0&4\end{pmatrix}$ is again triangular. Each expansion step just peels off the next diagonal entry.
...
1. $A = \begin{pmatrix}1&2&3\\2&5&3\\1&0&8\end{pmatrix}$. We reduce to upper triangular using only row-replacement operations, which do **not** change the determinant.
2. $R_2 \leftarrow R_2 - 2R_1$ gives new row 2 $= (0,\,1,\,-3)$. Determinant unchanged.
3. $R_3 \leftarrow R_3 - R_1$ gives new row 3 $= (0,\,-2,\,5)$. Determinant unchanged.
4. $R_3 \leftarrow R_3 + 2R_2$ gives new row 3 $= (0,\,0,\,-1)$. Determinant unchanged. The matrix is now $\begin{pmatrix}1&2&3\\0&1&-3\\0&0&-1\end{pmatrix}$.
5. Triangular, so $\det(A) = 1\cdot 1\cdot(-1) = -1$.
6. Sanity check by cofactor expansion along row 1: $1(40-0) - 2(16-3) + 3(0-5) = 40 - 26 - 15 = -1$. ✓ (If we had used a row *swap* or *scaling* here, we would have had to divide out the corresponding factor at the end.)

**Worked Example:**

**Using the product and inverse rules**

1. Suppose $\det(A) = 3$ and $\det(B) = -2$ for two $3\times 3$ matrices. Find $\det(A^2 B^{-1})$.
2. Product rule: $\det(A^2 B^{-1}) = \det(A)^2\,\det(B^{-1})$.
3. Inverse rule: $\det(B^{-1}) = 1/\det(B) = 1/(-2) = -\tfrac{1}{2}$ (valid because $\det(B)\neq 0$, so $B$ is invertible).
4. $\det(A^2 B^{-1}) = 3^2 \cdot (-\tfrac{1}{2}) = 9\cdot(-\tfrac{1}{2}) = -\tfrac{9}{2}$.


## Cramer's Rule {#cramer}

For a square system $A\mathbf{x}=\mathbf{b}$ with $\det(A)\neq 0$ (so $A$ is invertible and the solution is unique), Cramer's Rule gives each variable explicitly: $x_i = \det(A_i)/\det(A)$, where $A_i$ is the matrix $A$ with its $i$-th column replaced by $\mathbf{b}$. If $\det(A)=0$ the rule does not apply — the system has either no solution or infinitely many, and the division by zero signals exactly that failure.
...
1. Solve $3x+y=7$, $2x+5y=4$ using Cramer's Rule.
2. $A=\begin{pmatrix}3&1\\2&5\end{pmatrix}$, $\mathbf{b}=\begin{pmatrix}7\\4\end{pmatrix}$, $\det(A)=15-2=13$.
3. $x = \det\begin{pmatrix}7&1\\4&5\end{pmatrix}/13 = (35-4)/13 = 31/13$.
4. $y = \det\begin{pmatrix}3&7\\2&4\end{pmatrix}/13 = (12-14)/13 = -2/13$.
5. Sanity check by substituting back. Equation 1: $3\cdot\tfrac{31}{13} + \left(-\tfrac{2}{13}\right) = \tfrac{93-2}{13} = \tfrac{91}{13} = 7$. ✓ Equation 2: $2\cdot\tfrac{31}{13} + 5\cdot\left(-\tfrac{2}{13}\right) = \tfrac{62-10}{13} = \tfrac{52}{13} = 4$. ✓
...
The determinant is *multilinear* and *alternating* in the columns. "Multilinear" means it is linear in each column separately (holding the others fixed): scaling one column by $c$ scales $\det$ by $c$, and splitting one column into a sum splits the determinant into a sum. "Alternating" means swapping two columns flips the sign (and therefore two equal columns force $\det=0$). The parallelogram area interpretation makes this geometric: scaling a side scales the area; reflecting a side flips orientation.
...
**Area and orientation of a parallelogram**

1. Find the area and orientation of the parallelogram spanned by the columns $\mathbf{u}=\langle 3,1\rangle$ and $\mathbf{v}=\langle 1,2\rangle$.
2. Form $A=\begin{pmatrix}3&1\\1&2\end{pmatrix}$ (columns $\mathbf{u}$ and $\mathbf{v}$) and compute $\det(A) = 3\cdot 2 - 1\cdot 1 = 5$.
3. Area $= |\det(A)| = |5| = 5$.
4. Orientation: $\det(A) = 5 > 0$, so the ordered pair $(\mathbf{u},\mathbf{v})$ has the same (counterclockwise) orientation as the standard basis — no reflection.
5. Sanity check with a shear, which preserves area. Replace $\mathbf{v}$ by $\mathbf{v} - \tfrac{1}{3}\mathbf{u} = \langle 1-1,\ 2-\tfrac{1}{3}\rangle = \langle 0,\tfrac{5}{3}\rangle$. The sheared parallelogram spans $\langle 3,1\rangle$ and $\langle 0,\tfrac{5}{3}\rangle$, with $\det\begin{pmatrix}3&0\\1&\tfrac{5}{3}\end{pmatrix} = 3\cdot\tfrac{5}{3} - 0 = 5$. Same area. ✓
