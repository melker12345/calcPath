...
- Between-group sum of squares (each group's mean deviation from the grand mean, weighted by its size): $SS_{\text{between}} = 4[(5-6.33)^2+(8-6.33)^2+(6-6.33)^2] = 4[1.77+2.79+0.11] = 4(4.67) = 18.67$. Since there are $k=3$ groups, $df_{\text{between}}=k-1=2$.
- Suppose the pooled within-group scatter gives $SS_{\text{within}}=27$. Then $df_{\text{within}}=N-k=12-3=9$.
- Convert each SS to a mean square (a variance estimate) by dividing by its df: $MS_{\text{between}}=18.67/2=9.33$ and $MS_{\text{within}}=27/9=3$.
- $F=MS_{\text{between}}/MS_{\text{within}}=9.33/3=3.11$. Compare to the critical value $F_{0.05,\,2,\,9}=4.26$. Since $3.11<4.26$, we **fail to reject** $H_0$: the group means are not significantly different at $\alpha=0.05$.
- Sanity check: $F$ measures signal-to-noise. Here between-group variance ($9.33$) is only about $3\times$ the within-group noise ($3$), which sounds like a lot but is not enough for just $k=3$ groups and $9$ error df — the $F$-table demands more. As a further check, the effect size $\eta^2=SS_{\text{between}}/SS_{\text{total}}=18.67/(18.67+27)=18.67/45.67\approx 0.41$: about $41\%$ of the variation is between groups, a large descriptive effect that the small sample simply cannot certify.
...
**Worked Example: checking the equal-variance rule of thumb**
- Three groups have sample standard deviations $s_1=4.0$, $s_2=6.0$, $s_3=7.5$.
- Apply the rule of thumb: compare the largest SD to the smallest. Ratio $=s_{\max}/s_{\min}=7.5/4.0=1.875$.
- Since $1.875\leq 2$, the spreads are close enough that ordinary one-way ANOVA is reasonable — no red flag on homoscedasticity.
- Now suppose group 3 instead had $s_3=9.0$. Ratio $=9.0/4.0=2.25>2$: this exceeds the threshold, so you would confirm with Levene's test and, if it is significant, switch to **Welch's ANOVA** (does not assume equal variances) rather than trusting the standard $F$-test.
- Sanity check: the rule compares standard deviations, but ANOVA works with *variances*. A SD ratio of $2$ is a variance ratio of $2^2=4$ — so this "ratio $\leq 2$" guideline is already fairly permissive.
...
**Worked Example: Bonferroni vs. the inflated error rate**
- You have $k=4$ groups and a significant ANOVA. You want to compare every pair.
- Number of pairwise comparisons: $\binom{4}{2}=\dfrac{4\cdot 3}{2}=6$.
- If you ran all $6$ tests at $\alpha=0.05$ with no correction, the family-wise error rate (chance of *at least one* false positive, assuming independent tests) is $1-(1-0.05)^6 = 1-0.95^6 = 1-0.735 = 0.265$ — about a $26.5\%$ chance of a spurious "difference." That is why a correction is needed.
- Bonferroni fix: test each pair at $\alpha^*=\alpha/m=0.05/6\approx 0.0083$. A pair is declared different only if its $p$-value falls below $0.0083$.
...
**Worked Example: degrees of freedom and reading an interaction**
