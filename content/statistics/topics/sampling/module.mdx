...
**Worked Example: parameter vs. statistic, and unbiasedness**
...
- Now draw the sample $\{80, 100\}$. The sample mean is a statistic: $\bar{x} = (80+100)/2 = 90$. Draw instead $\{70, 90\}$: now $\bar{x} = 80$. Same population, different samples, different statistics — that is why $\bar{X}$ is a random variable while $\mu$ is a fixed number.
- Unbiasedness means the statistic is right *on average across all possible samples*, not on any single one. There are $\binom{5}{2}=10$ possible samples of size $2$. Averaging all ten sample means gives back $\mu=90$ exactly — that is what $E(\bar{X}) = \mu$ asserts.
- Sanity check: not every sample mean equals $\mu$ (e.g. $\{70,90\}$ gives $\bar{x}=80$), yet the average of all ten sample means lands exactly on $\mu=90$. Unbiased describes the estimator's long-run centre, not its accuracy on one draw.
...
**Worked Example: why the average tightens but the count doesn't**
...
- The *proportion* of heads is exactly the sample mean $\bar{X}_n$, so the LLN says $\bar{X}_n \to 0.5$. Its spread is $SE = \sigma/\sqrt{n}$ with $\sigma = \sqrt{0.5 \times 0.5} = 0.5$. For $n=100$: $SE = 0.5/\sqrt{100} = 0.05$. For $n=10{,}000$: $SE = 0.5/\sqrt{10{,}000} = 0.005$ — ten times tighter.
- A common trap: the *count* of heads does not converge to $n/2$. At $n=10{,}000$ the standard deviation of the count is $\sigma\sqrt{n} = 0.5\times 100 = 50$, so being off by $50$-plus heads is routine. It is the *proportion* $50/10{,}000 = 0.005$ that is tiny.
- Sanity check: the LLN promises the average nears the truth, not that heads and tails literally even out in count. As $n$ grows the count typically drifts *further* from exactly $n/2$ in absolute terms, even as the proportion homes in on $0.5$ — which is precisely why the gambler's fallacy is wrong.
...
**Worked Example: CLT probability for a sample mean**
...
**Worked Example: sample proportion, condition check, and margin of error**
- A poll of $n = 400$ voters finds $\hat{p} = 0.45$ support. Build a $95\%$ confidence interval for the true proportion $p$.
- First check the CLT condition. We don't know $p$, so use $\hat{p}$: $n\hat{p} = 400(0.45) = 180 \geq 10$ and $n(1-\hat{p}) = 400(0.55) = 220 \geq 10$. Both pass, so the normal approximation is justified.
- Standard error: $SE = \sqrt{\hat{p}(1-\hat{p})/n} = \sqrt{0.45 \times 0.55 / 400} = \sqrt{0.2475/400} = \sqrt{0.00061875} \approx 0.02488$.
- Margin of error at $95\%$ uses $z^\* = 1.96$: $ME = 1.96 \times 0.02488 \approx 0.0488$.
- Confidence interval: $0.45 \pm 0.0488 = (0.401,\ 0.499)$.
- Sanity check: $0.05$ is a believable margin for a poll of a few hundred, and the interval sits just below $0.5$, matching the observed $\hat{p} = 0.45$. Note we did *not* apply the FPC: the voter population vastly exceeds $400$, so $n/N$ is far below $5\%$.
