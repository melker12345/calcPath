...
- Pool and rank all $6$ values from smallest to largest. Since the values $1,2,3,4,5,6$ are already in order, each value's rank equals itself: A gets ranks $1,3,5$ and B gets ranks $2,4,6$.
- Rank sums: $R_1 = 1+3+5 = 9$ (group A), $R_2 = 2+4+6 = 12$ (group B).
- $U_1 = R_1 - \dfrac{n_1(n_1+1)}{2} = 9 - \dfrac{3(4)}{2} = 9 - 6 = 3$.
- $U_2 = R_2 - \dfrac{n_2(n_2+1)}{2} = 12 - 6 = 6$ (or directly $U_2 = n_1 n_2 - U_1 = 9 - 3 = 6$).
- Sanity check: $U_1 + U_2 = 3 + 6 = 9 = n_1 n_2 = 3\times 3$. ✓ Report $U = \min(U_1,U_2) = 3$; with such tiny samples this is far from significant.

**Worked Example: Wilcoxon signed-rank by hand**
- Before/after scores for $5$ subjects, $d_i = \text{after} - \text{before}$: $d = \{+4,\ -1,\ +3,\ 0,\ +6\}$.
- Discard the pair with $d_i = 0$; four differences remain: $\{+4,\ -1,\ +3,\ +6\}$.
- Rank the absolute values $\{4,1,3,6\}$ from smallest to largest: $|{-1}|\to 1$, $|{+3}|\to 2$, $|{+4}|\to 3$, $|{+6}|\to 4$.
- Reattach signs: positive ranks are $2,3,4$ (from $+3,+4,+6$); the only negative rank is $1$ (from $-1$).
- $W^+ = 2+3+4 = 9$, $W^- = 1$.
- Sanity check: $W^+ + W^- = 9 + 1 = 10 = \dfrac{m(m+1)}{2} = \dfrac{4(5)}{2}$ for the $m=4$ retained pairs. ✓ The large imbalance ($9$ vs. $1$) points to a positive shift.
...
**Worked Example: Kruskal-Wallis $H$**
- Three groups, each of size $n_j = 4$, total $N = 12$. Pooled rank sums: $R_1 = 30$, $R_2 = 24$, $R_3 = 24$.
- Sanity check on the ranks: the total of all ranks must be $\frac{N(N+1)}{2} = \frac{12\cdot 13}{2} = 78$, and indeed $30 + 24 + 24 = 78$. ✓
- $\sum_j \dfrac{R_j^2}{n_j} = \dfrac{30^2}{4} + \dfrac{24^2}{4} + \dfrac{24^2}{4} = \dfrac{900 + 576 + 576}{4} = \dfrac{2052}{4} = 513$.
- $H = \dfrac{12}{N(N+1)}\cdot 513 - 3(N+1) = \dfrac{12}{12\cdot 13}\cdot 513 - 3(13) = \dfrac{513}{13} - 39 = 39.46 - 39 = 0.46$.
- $df = k-1 = 2$, so compare to $\chi^2_{0.05,2} = 5.99$. Since $0.46 < 5.99$, fail to reject $H_0$: no evidence the groups differ.

**Worked Example: Spearman's $r_s$**
- Five students ranked on a midterm ($x$) and a final ($y$): $x$-ranks $= 1,2,3,4,5$ and $y$-ranks $= 2,4,1,3,5$.
- Rank differences $d_i = x_i - y_i$: $-1,\ -2,\ +2,\ +1,\ 0$.
- Squared differences $d_i^2$: $1,\ 4,\ 4,\ 1,\ 0$, so $\sum d_i^2 = 10$.
- No ties, so use the shortcut with $n = 5$: $r_s = 1 - \dfrac{6\sum d_i^2}{n(n^2-1)} = 1 - \dfrac{6(10)}{5(25-1)} = 1 - \dfrac{60}{120} = 0.5$.
- Sanity check: $0.5 \in [-1, 1]$ and is positive, matching the fact that the two rankings broadly agree (both start low and end at $5$). ✓
...
**Worked Example: Choosing the right test**
...
- Scenario B: same patients' symptom scores measured before and after a treatment (paired), scores skewed. Paired design with non-normal differences $\Rightarrow$ **Wilcoxon signed-rank**.
- Scenario C: comparing recovery time across four hospitals, one hospital has an extreme outlier. Three or more independent groups with an outlier $\Rightarrow$ **Kruskal-Wallis** (then Dunn's test if significant).
- Scenario D: relating exam rank to hours-studied rank, monotone but curved. Association of two quantitative variables without a linear form $\Rightarrow$ **Spearman's $r_s$**.
- Reasoning pattern: identify (i) how many groups / whether paired, and (ii) whether parametric assumptions hold; the two answers pick the test.
