...
**Worked Example: Reading a process vs. a single variable**
- Let $X_t$ be the number of customers in a shop at hour $t$, for $t \in \{9, 10, \dots, 17\}$ (a discrete-time, discrete-state process).
- A single random variable answers "how many customers at 2 pm?" — that is just $X_{14}$, described by one distribution.
- The *process* answers richer questions that involve several times at once, e.g. "given $X_{14} = 20$, what is the distribution of $X_{15}$?" (a transition) or "what fraction of the day is the shop empty?" (long-run behaviour). These need the *joint* distribution of $(X_9, \dots, X_{17})$, not just one marginal.
- Sanity check: if the $X_t$ were all independent, knowing $X_{14}$ would tell us nothing about $X_{15}$. The whole point of a process is that the dependence structure across $t$ carries information — that is what we model.
...
**Worked Example: Two-state weather Markov chain**
...
- Transition matrix $P = \begin{bmatrix} 0.8 & 0.2 \\ 0.4 & 0.6 \end{bmatrix}$. (Each row sums to 1, as required.)
- If today is sunny, the probability of sunny two days from now is the (Sunny, Sunny) entry of $P^2$. Compute it as a dot product of row 1 of $P$ with column 1 of $P$: $(P^2)_{11} = 0.8\times 0.8 + 0.2\times 0.4 = 0.64 + 0.08 = 0.72$.
- So $P(\text{sunny in 2 days}\mid \text{sunny today}) = 0.72$.
- Sanity check: the full second row of $P^2$ must also sum to 1. The other first-row entry is $(P^2)_{12} = 0.8\times 0.2 + 0.2\times 0.6 = 0.16 + 0.12 = 0.28$, and $0.72 + 0.28 = 1$ ✓.

**Worked Example: Finding the stationary distribution (weather example)**
...
- Writing $\pi P = \pi$ column by column gives: $\pi_S = 0.8\pi_S + 0.4\pi_R$ (column Sunny) and $\pi_R = 0.2\pi_S + 0.6\pi_R$ (column Rainy).
- Take the first equation: $\pi_S - 0.8\pi_S = 0.4\pi_R$, i.e. $0.2\pi_S = 0.4\pi_R$, so $\pi_S = 2\pi_R$. (The two equations are linearly dependent — one is redundant — which is why we need the normalization $\pi_S + \pi_R = 1$ to pin down a unique answer.)
- Substituting: $2\pi_R + \pi_R = 1 \Rightarrow \pi_R = 1/3$, hence $\pi_S = 2/3$.
- Sanity check: verify $\pi P = \pi$ directly. First component: $0.8\cdot\tfrac{2}{3} + 0.4\cdot\tfrac{1}{3} = \tfrac{1.6}{3} + \tfrac{0.4}{3} = \tfrac{2}{3} = \pi_S$ ✓.
...
**Worked Example: Calls to a call center**
...
- For a Poisson process, the number of events in an interval of length $t$ is Poisson with mean $\lambda t$. Keep the time units consistent: with $\lambda = 12$ per hour and $t = 0.5$ hours, the mean is $\lambda t = 12 \times 0.5 = 6$ calls.
- Plug $k = 5$ into the Poisson pmf: $P(X = 5) = \dfrac{e^{-6}\, 6^{5}}{5!} = \dfrac{e^{-6}\cdot 7776}{120}$.
- Numerically $e^{-6} \approx 0.002479$, so $P(X=5) \approx \dfrac{0.002479 \times 7776}{120} \approx 0.1606$.
- So there is about a 16% chance of exactly 5 calls in 30 minutes.
- Sanity check: the mean is 6, and for a Poisson the most likely counts cluster around the mean, so a value of 5 should carry a moderate probability of order 0.15 — consistent with what we got. (Had we forgotten to convert 30 minutes to $t=0.5$ and used mean 12, the answer would be a tiny $\approx 0.013$, a red flag that the units were wrong.)

**Worked Example: Superposition and thinning (two coffee shops)**
...
- Now thin the merged stream: label each arriving customer 'takeout' independently with probability $p = 0.25$. The takeout stream is Poisson with rate $p\lambda = 0.25 \times 8 = 2$/hr, and the dine-in stream is Poisson with rate $(1-p)\lambda = 0.75 \times 8 = 6$/hr; crucially, these two thinned streams are *independent* of each other.
- Sanity check: the two thinned rates must add back to the original, $2 + 6 = 8$ ✓. And the expected number of takeout customers in an 8-hour day is $2 \times 8 = 16$.
- This is why Poisson processes are so convenient: merging and splitting preserve the Poisson property (independence of increments is what makes both work).
...
**Worked Example: Simulating a stock price path (GBM)**
...
- The Itô drift correction is $\mu - \tfrac{\sigma^2}{2} = 0.08 - \tfrac{0.2^2}{2} = 0.08 - 0.02 = 0.06$. A single realization is $S_1 = 100\exp\!\big(0.06\cdot 1 + 0.2\,Z\big)$.
- Concrete draw: suppose $Z = +1$ (a one-standard-deviation up move). Then $S_1 = 100\exp(0.06 + 0.2) = 100\exp(0.26) \approx 100 \times 1.2969 = 129.69$. A draw of $Z = -1$ instead gives $S_1 = 100\exp(0.06 - 0.2) = 100\exp(-0.14) \approx 86.94$ — note the up and down moves are *not* symmetric around 100, because exp is convex; that is the log-normal shape.
- Expected ending price: for $S_1 = S_0\exp((\mu-\tfrac{\sigma^2}{2})t + \sigma W_t)$, the $-\tfrac{\sigma^2}{2}$ correction is exactly what makes $E[S_t] = S_0 e^{\mu t}$. Here $E[S_1] = 100\,e^{0.08} \approx 108.33$.
- Sanity check on why the correction is needed: for $X \sim N(m, v)$, $E[e^{X}] = e^{m + v/2}$. With $m = (\mu-\tfrac{\sigma^2}{2})t$ and $v = \sigma^2 t$, the exponent becomes $(\mu-\tfrac{\sigma^2}{2})t + \tfrac{\sigma^2 t}{2} = \mu t$, so the $\sigma^2/2$ terms cancel and the mean grows cleanly at rate $\mu$ ✓. Without the correction, the mean would be inflated by a factor $e^{\sigma^2 t/2}$.
...
**Worked Example: AR(1) autocorrelation structure**
...
- Derive the variance $\gamma_0 = \mathrm{Var}(X_t)$. By stationarity $\mathrm{Var}(X_t) = \mathrm{Var}(X_{t-1}) = \gamma_0$. Taking the variance of both sides of the AR(1) equation and using that $\epsilon_t$ is independent of $X_{t-1}$: $\gamma_0 = \phi^2 \gamma_0 + \sigma^2$. Solving, $\gamma_0(1-\phi^2) = \sigma^2$, hence $\gamma_0 = \dfrac{\sigma^2}{1-\phi^2}$.
- So ACF at lag 1 is $\phi$, at lag 2 is $\phi^2$, etc. — geometric decay. When $\phi > 0$, $\gamma_0 > \sigma^2$: positive autocorrelation (persistence) amplifies variability above the raw innovation variance.
- Numeric instance: with $\phi = 0.6$ and $\sigma^2 = 4$, $\gamma_0 = \dfrac{4}{1 - 0.6^2} = \dfrac{4}{1 - 0.36} = \dfrac{4}{0.64} = 6.25$, and $\rho_1 = 0.6$, $\rho_2 = 0.36$.
- Sanity check: $\gamma_0 = 6.25 > 4 = \sigma^2$ ✓, as expected for positive $\phi$; and as $\phi \to 0$ the formula returns $\gamma_0 \to \sigma^2$ (pure white noise, no persistence) ✓.
...
**Worked Example: Long-run replacement rate (renewal theorem)**
...
- The **elementary renewal theorem** says the long-run rate of renewals is the reciprocal of the mean inter-renewal time: $\displaystyle \lim_{t\to\infty} \frac{N(t)}{t} = \frac{1}{\mu}$, where $N(t)$ counts renewals up to time $t$. (This requires $0 < \mu < \infty$; the individual lifetime distribution need not be exponential — that is the payoff of renewal theory over the Poisson process.)
- Here the long-run replacement rate is $1/\mu = 1/4 = 0.25$ parts per year, i.e. one replacement every 4 years on average, so about $0.25 \times 40 = 10$ replacements over a 40-year horizon.
- Sanity check: the Poisson process is the special case with exponential lifetimes of mean $1/\lambda$; there $1/\mu = \lambda$, recovering the familiar Poisson rate ✓. The theorem generalizes that fact to *any* i.i.d. lifetime distribution.

**Worked Example: Why a Poisson process is a Lévy process (but a renewal process need not be)**
...
- Poisson process: the count in $(s, s+h]$ is Poisson$(\lambda h)$ regardless of $s$ (stationary) and independent of counts on disjoint intervals (independent) — so it qualifies. Brownian motion qualifies for the same two reasons with Gaussian increments.
- A general renewal process with, say, uniform$[2,6]$ lifetimes does *not* have independent increments: knowing a renewal just happened at time $s$ changes the distribution of how long until the next one, so counts over adjacent intervals are dependent. This is exactly why only the *exponential*-lifetime renewal process (the Poisson process) is a Lévy process — the memoryless property is what restores independent increments.
