...
**Worked Example: Expected value and variance of a PMF**
- A game pays \$10 with probability $0.1$, \$5 with probability $0.3$, and -\$2 with probability $0.6$.
- First check the PMF is valid: the probabilities are all $\geq 0$ and $0.1+0.3+0.6 = 1$ ✓.
- $E(X) = 10(0.1)+5(0.3)+(-2)(0.6) = 1+1.5-1.2 = 1.3$.
- For the variance, use $\text{Var}(X)=E(X^2)-[E(X)]^2$, which is usually less error-prone than the definition $E[(X-\mu)^2]$.
- $E(X^2) = 100(0.1)+25(0.3)+4(0.6) = 10+7.5+2.4 = 19.9$.
- $\text{Var}(X) = 19.9 - 1.3^2 = 19.9-1.69 = 18.21$. $\sigma = \sqrt{18.21}\approx 4.27$.
- Sanity check: the spread of payouts runs from $-2$ to $10$, so a standard deviation of about $4.3$ is a believable "typical distance from $1.3$".

**Worked Example: Transforming a random variable**
...
- A game pays $Y = 2X + 1$ dollars. Find $E(Y)$ and $\text{Var}(Y)$.
- Using $E(aX+b)=aE(X)+b$: $E(Y)=2(3.5)+1 = 8$.
- Using $\text{Var}(aX+b)=a^2\text{Var}(X)$ (the $+b$ shift does not change spread): $\text{Var}(Y)=2^2(2.917) \approx 11.67$.
- Sanity check: doubling the payout doubles the mean-plus-constant and multiplies the variance by $2^2=4$, not by $2$ — spread scales with the *square* of the multiplier.
...
**Worked Example: Binomial probability — exactly $k$ successes**
- A fair coin is flipped $5$ times. Find $P(X=3)$.
- Check BINS: two outcomes (H/T), independent flips, $n=5$ fixed, $p=0.5$ constant ✓ — the binomial applies.
...
**Worked Example: A cumulative probability with the complement**
- For $X\sim B(6,0.3)$, find $P(X\geq 2)$ — the chance of at least two successes.
- Summing $P(X=2)+P(X=3)+\cdots+P(X=6)$ is five terms; the complement $P(X\geq 2)=1-P(X=0)-P(X=1)$ is only two, so use that.
- $P(X=0)=\binom{6}{0}(0.3)^0(0.7)^6 = (0.7)^6 \approx 0.1176$.
- $P(X=1)=\binom{6}{1}(0.3)^1(0.7)^5 = 6(0.3)(0.16807) \approx 0.3025$.
- $P(X\geq 2) = 1 - 0.1176 - 0.3025 = 0.5799$.
- Sanity check: the mean is $\mu=np=1.8$, so "at least $2$" straddles the mean and a probability near $0.58$ is reasonable.
...
**Worked Example: Poisson probability**
...
**Worked Example: "At least one" via the complement**
- Accidents occur at $\lambda=2$ per week. Find $P(X\geq 1)$ in a given week.
- "At least one" spans infinitely many counts ($1,2,3,\ldots$), but its complement is the single term $P(X=0)$, so compute $P(X\geq 1)=1-P(X=0)$.
- $P(X=0)=e^{-2}\cdot 2^0/0! = e^{-2} \approx 0.1353$.
- $P(X\geq 1) = 1 - 0.1353 = 0.8647$.
- Sanity check: with an average of $2$ events per week, a week with zero events should be uncommon, so a large value like $0.86$ is expected.

**Worked Example: Poisson approximation to the binomial**
- A part is defective with probability $p=0.03$; a lot has $n=100$ parts. Approximate $P(X=0)$ (no defectives).
- Here $n$ is large and $p$ is small, so set $\lambda = np = 100(0.03) = 3$ and treat $X$ as approximately Poisson($3$).
- $P(X=0) \approx e^{-3} \approx 0.0498$.
- Sanity check against the exact binomial: $P(X=0)=(1-0.03)^{100}=(0.97)^{100} \approx 0.0476$. The Poisson value $0.0498$ is within about $0.002$ — a good approximation, and far quicker than the exact power.
...
**Worked Example: Geometric probability, mean, and variance**
...
- $P(X=3) = (1-p)^{3-1}p = (0.75)^2(0.25) = 0.5625 \times 0.25 = 0.140625$ — two misses, then a make.
- Mean: $E(X) = 1/p = 1/0.25 = 4$. On average it takes $4$ shots to get the first make.
- Variance: $\text{Var}(X) = (1-p)/p^2 = 0.75/0.0625 = 12$.
- Sanity check: $P(X>3) = (1-p)^3 = 0.75^3 \approx 0.422$, so there is still a $42\%$ chance the first make has not happened by the third shot — consistent with a mean of $4$.

**Worked Example: Negative binomial**
- Using the same shooter ($p=0.5$ now, for a cleaner number), let $X$ be the shot on which she records her $r=3$rd make. Find $P(X=5)$.
- The 5th shot must be the 3rd make, and exactly $2$ of the first $4$ shots are makes: $P(X=5) = \binom{4}{2}(0.5)^{2}(0.5)^{3} = 6(0.25)(0.125) = 0.1875$.
- Check the mean: $E(X) = r/p = 3/0.5 = 6$, so needing exactly $5$ trials (just under the mean) being fairly likely is reasonable.
...
**Worked Example: Picking the distribution from the wording**
...
- Scenario B: "A page averages $0.5$ typos. What is the chance a page has no typos?" You are counting rare events in a fixed window (one page) at a constant rate → **Poisson**, $\lambda=0.5$, giving $P(X=0)=e^{-0.5}\approx 0.607$.
- Scenario C: "How many pages must an editor read before finding the first typo?" You are counting trials until the first success → **geometric**.
- Scenario D: "A box holds $20$ fuses, $4$ of them defective; you grab $3$ without replacing. How many are defective?" Sampling without replacement from a small population → **hypergeometric**, not binomial, because $p$ changes after each draw.
- The tells: *fixed count of trials* → binomial; *rate over an interval* → Poisson; *wait until first success* → geometric; *drawing from a small finite pool without replacement* → hypergeometric.
