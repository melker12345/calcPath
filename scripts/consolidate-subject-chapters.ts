 ...
 * Run: npx tsx scripts/consolidate-subject-chapters.ts <algebra|geometry|precalculus>
 */
...
async function main() {
  ...
  if (!subject || !(subject in MAPS)) {
    console.error(
      "Usage: npx tsx scripts/consolidate-subject-chapters.ts <algebra|geometry|precalculus>",
    );
    process.exit(1);
  }
